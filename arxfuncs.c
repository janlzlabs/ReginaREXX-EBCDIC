...
streng *arexx_readch( tsd_t *TSD, cparamboxptr parm1 )
{
  ...
  else
  {
    ...
    count = fread( Str_val(ret)/*ret->value*/, 1, count, file );
    if ( count == -1 )
    {
       /*
...
streng *arexx_bitchg( tsd_t *TSD, cparamboxptr parm1 )
{
  ...
  Str_val(ret)/*ret->value*/[byte]^=(char)(1<<dt.rem);
...
streng *arexx_bitcomp( tsd_t *TSD, cparamboxptr parm1 )
{
  ...
  for ( cp1=Str_val(s1)/*s1->value*/+s1->len-1, cp2=Str_val(s2)/*s2->value*/+s2->len-1, i=0;
       cp2 >= Str_val(s2)/*s2->value*/;
       cp1--, cp2--, i++ )
  {
    if ( *cp1 != *cp2 )
  ...
  if ( parm3==NULL || parm3->value==NULL || parm3->value->len==0 )
    ...
    pad = Str_val(parm3->value)/*parm3->value->value*/[0];

  for ( ;
  cp1 >= Str_val(s1)/*s1->value*/;
  cp1--, i++ )
  {
    if ( *cp1 != pad )
...
streng *arexx_hash( tsd_t *TSD, cparamboxptr parm1 )
{
  ...
  uc = (unsigned char *)Str_val(parm1->value)/*parm1->value->value*/;
...
streng *arexx_compress( tsd_t *TSD, cparamboxptr parm1 )
{
  ...
  for ( i=start=0; i<ret->len; i++ )
  {
    ...
    if ( strchr( match, Str_val(ret)/*ret->value*/[i] )==NULL )
    {
      Str_val(ret)/*ret->value*/[start] = Str_val(ret)/*ret->value*/[i];
      start++;
    }
  }
  ret->len = start;
...
#ifdef REXX_8BIT_SUPPORT
static const streng T_str = { 1, 1, 0, "T" };
#else
...
streng *arexx_upper( tsd_t *TSD, cparamboxptr parms )
{
    ...
    * Change them
    */
   mem_upper( &Str_val(ptr)/*ptr->value*/[i], changecount );
   /*
    * Append pad characters if required...
    */
   if (changecount < length)
      memset(&Str_val(ptr)/*ptr->value*/[changecount], padch, length - changecount);
   /*
...
streng *arexx_export( tsd_t *TSD, cparamboxptr parm1 )
{
  ...
  memptr = *((void **)Str_val(parm1->value)/*parm1->value->value*/);
  ...
  if ( parm4 == NULL || parm4->value == NULL || parm4->value->len == 0 )
    ...
    fill = Str_val(parm4->value)/*parm4->value->value*/[0];

  if (len > src->len)
  {
    memcpy( memptr, Str_val(src)/*src->value*/, src->len );
    memset( ((char *)memptr)+src->len, fill, len - src->len );
  }
  else
    memcpy( memptr, Str_val(src)/*src->value*/, len );
