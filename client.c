...
static streng *wrapstring( const tsd_t *TSD, const char *string, int length )
{
   ...
   if (length>=0 && string)
   {
      ...
      // need to populate streng->value, then base_value will be converted to base encoding, so we set converted bit
      memcpy( Str_origval(result)/*result->value*/, string, length ) ;
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(*command);
   streng_clone_value(*command);
   Str_restore_TSD(*command);

   streng_alloc_base_value(*environment);
   streng_clone_value(*environment);
   Str_restore_TSD(*environment);
#endif
...
#ifdef REXX_8BIT_SUPPORT
      streng_alloc_base_value(h);
      streng_clone_value(h);
      Str_restore_TSD(h);
#endif
                   ...
                   unsigned long *instore_length )
{
   ...
   if ( ( old_exit_addr = TSD->systeminfo->script_exit ) == NULL )
   {
      ...
      if ( setjmp( *TSD->systeminfo->script_exit ) )
      {
         ...
         {
            if ( result )
               RetCode = atoi( Str_val(result)/*->value*/ );
            else
               RetCode = -1;
         }
                          ...
                          int *allocated )
{
   ...
   strncat( ctmp, Str_val(TSD->systeminfo->input_file)/*->value*/, infile ) ;
...
static int handle_param( tsd_t *TSD, int *Length, char **String )
{
   ...
   {
      *(Length+1) = value->len ;
      *(String+1) = Str_val(value)/*value->value*/ ;
   }
   else
   ...
   if (rval)
   {
      if (rval->stem)
      {
         *allocated |= 1;
         ...
         memcpy(Strings[0], Str_val(rval->stem->name)/*rval->stem->name->value*/, rval->stem->name->len);
         memcpy(Strings[0]+rval->stem->name->len,
                                      Str_val(rval->name)/*rval->name->value*/, rval->name->len ) ;
      }
      else
      {
         Lengths[0] = rval->name->len ;
         Strings[0] = Str_val(rval->name)/*rval->name->value*/ ;
      }

      if (rval->value)
      {
         Lengths[1] = rval->value->len ;
         Strings[1] = Str_val(rval->value)/*->value*/ ;
      }
      else
      {
         *allocated |= 2;
         ...
         memcpy( Strings[1], Str_val(rval->stem->name)/*rval->stem->name->value*/, value->stem->name->len );
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(outdata);
   streng_clone_value(outdata);
   Str_restore_TSD(outdata);
#endif
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(outdata1);
   streng_clone_value(outdata1);
   Str_restore_TSD(outdata1);

   streng_alloc_base_value(outdata2);
   streng_clone_value(outdata2);
   Str_restore_TSD(outdata2);
#endif
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(*indata);
   streng_clone_value(*indata);
   Str_restore_TSD(*indata);
#endif
...
streng *SubCom( tsd_t *TSD, const streng *command, const streng *envir, int *rc )
{
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(envir);
   streng_clone_value(envir);
   Str_restore_TSD(envir);
   ...
   /* Need to pass originally encoded command and envir
    *
    */
   *rc = IfcSubCmd( TSD,
                    envir ? envir->len : RX_NO_STRING,
                    envir ? Str_origval(envir)/*envir->value*/ : NULL,
                    command->len, Str_origval(command)/*command->value*/, &tmplen, &tmpptr ) ;
                        ...
                        int *allocated)
{
   ...
   {
      Lengths[1] = value->len;
      Strings[1] = (char *) Str_origval(value)/*value->value*/; // TODO origval
      *allocated = 0;
      Free_stringTSD( varbl );
      return RX_CODE_OK;
   }
   ...
   memcpy( retval, Str_origval(value)/*value->value*/, len ); // TODO origval
   *allocated = 2;
   Free_stringTSD( varbl );
   ...
   else
   {
       ...
       */
      if ( len )
         retval[0] = Char_convert((unsigned char) rx_toupper( Char_restore(retval[0]) ));
      for( i = 1; ( i < len ) && ( Char_convert('.') != retval[i] ); i++ )
         retval[i] = Char_convert((unsigned char) rx_toupper( Char_restore(retval[i]) ));
   }
...
static int SetVariable( tsd_t *TSD, int Code, int *Lengths, char *Strings[] )
{
   ...
   else
   {
       ...
       */
      if ( varbl->len && Str_val(varbl)/*varbl->value*/[0] == '.' )
      {
         Free_stringTSD( varbl ) ;
         return RX_CODE_INVNAME;
      }
       ...
       */
      if ( varname->len )
         Str_val(varname)/*varname->value*/[0] = (unsigned char) rx_toupper( Str_val(varname)/*varname->value*/[0] );
      for( i = 1; ( i < varname->len ) && ( '.' != Str_val(varname)/*varname->value*/[i] ); i++ )
      {
         Str_val(varname)/*varname->value*/[i] = (unsigned char) rx_toupper( Str_val(varname)/*varname->value*/[i] );
      }

#ifdef REXX_8BIT_SUPPORT
      streng_clone_base_value(varname);
      Str_convert_TSD(varname);
#endif
   }
   Free_stringTSD( varbl ) ;
...
/****************************************************************************
 * Function: IfcDataStack - RexxDataStack implementation
 *
 * Description:
 *   This function implements the services of the SAA-like RexxDataStack
 *   function. The caller provides the appropriate TSD describing the 
 *   current environment. 
 *
 * Outputs:
 *   The status codes returned by stack_lifo and stack_fifo a varied and
 *   undisciplined...
 *
 ****************************************************************************/
int IfcDataStack(tsd_t *TSD, int Code, char **data, unsigned long *datalen, int* auxstatus)
{
   int      rc = 0;
   int      bRead, bWrite;
   streng   *strdata;

   bRead  = (Code == RX_STACK_PULL);
   bWrite = (Code == RX_STACK_PUSH) || (Code == RX_STACK_QUEUE);

   if (bWrite)  {
      /* stack will own strdata structure, dont free */
      strdata = Str_makeTSD(*datalen);

#ifdef REXX_8BIT_SUPPORT
      streng_alloc_base_value(strdata);
      streng_clone_value(strdata);
#endif

      memcpy(Str_origval(strdata)/*strdata->value*/, *data, *datalen); // TODO origval
      strdata->len = *datalen;

#ifdef REXX_8BIT_SUPPORT
      SET_STRENG_CONVERTED(strdata->converted);
      streng_clone_value(strdata);
      Str_restore_TSD(strdata);
#endif

      if (Code == RX_STACK_PUSH)
         rc = stack_lifo(TSD, strdata, NULL);

      if (Code == RX_STACK_QUEUE)
         rc = stack_fifo(TSD, strdata, NULL);
   }

   else if (Code == RX_STACK_PULL)  {
      strdata = popline(TSD, NULL, NULL, 0);
      *datalen = Str_len(strdata);
      ...
      *data = (char*) MallocTSD(*datalen);
      memcpy(*data, Str_origval(strdata)/*strdata->value*/, *datalen); // TODO origval
      Free_stringTSD(strdata);
   }
    
   else if (Code == RX_STACK_QUEUED)  
      *auxstatus = lines_in_stack(TSD, NULL);

// return  0;           /* stack.c return codes are an undoc'd nightmare ???????? */
   return rc;
}
                               ...
                               char called )
{
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(ExeName);
   streng_clone_value(ExeName);
   Str_restore_TSD(ExeName);
#endif
   ...
   else
   {
      ...
      memcpy( Str_val(retval)/*retval->value*/, RetString, RetLength ) ;
      FreeTSD( RetString );
...
int IfcCreateQueue( tsd_t *TSD, const char *qname, const int qlen, char *data, unsigned long *dupflag, unsigned long buflen )
{
...
#ifdef REXX_8BIT_SUPPORT
      streng_alloc_base_value(queuename);
      streng_clone_value(queuename);
#endif

      memcpy( Str_val(queuename)/*queuename->value*/, qname, qlen ) ;
      queuename->len = qlen;

#ifdef REXX_8BIT_SUPPORT
      streng_clone_base_value(queuename);
      Str_convert_TSD(queuename);
#endif
   }
   ...
   if ( rc == 0 || rc == 1 )
   {
       ...
       */
      memcpy( data, Str_val(strdata)/*strdata->value*/, min( strdata->len, (int) buflen - 1 ) );
...
int IfcAddQueue( tsd_t *TSD, const char *qname, const int qlen, const char *data, const int datalen, unsigned long addlifo )
{
   ...
   memcpy( Str_val(strdata)/*->value*/, data, datalen ) ;
   strdata->len = datalen;

#ifdef REXX_8BIT_SUPPORT
   streng_clone_base_value(strdata);
#endif
   Str_convert_TSD(strdata);
      ...
      else
      {
         ...
         if ( p == NULL )
         ...
         else
            {
                ...
                */
               *data = p;
               memcpy( *data, Str_origval(strdata)/*->value*/, len ); // TODO origval
               (*data)[len] = '\0';
               *datalen = len;
            }
         Free_stringTSD( strdata );
