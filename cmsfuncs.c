...
streng *cms_justify( tsd_t *TSD, cparamboxptr parms )
{
   ...
   streng_alloc_base_value(parms->value);
   streng_clone_value(parms->value);
   Str_restore_TSD(parms->value);
#endif

   cptr = Str_val(parms->value)/*parms->value->value*/ ;
   ...
   count = 0 ;
   out = Str_val(result)/*result->value*/ ;
   oend = out + length ;
   cp = cptr ;
   ...
   assert( out - Str_val(result)/*result->value*/ == length ) ;
   result->len = length ;
