...
static streng *unpack_hex( const tsd_t *TSD, const streng *string )
{
   ...
   res_ptr = Str_val(result)/*result->value*/ ;
    ...
    */
   end_ptr = (unsigned char *)(Str_origend(string)) ;
   ptr = (unsigned char *)(Str_origval(string)/*string->value*/) ;
   for (; ptr<end_ptr; ptr++ )
   {
    ...
    */
   result->len = ( res_ptr - Str_val(result)/*result->value*/ ) ;
...
static streng *pack_hex( tsd_t *TSD, const char *bif, const streng *string )
{
    ...
    */
   ptr = Str_val(string)/*string->value*/ ;
   ...
   byte_boundary = !((ptr-Str_val(string)/*string->value*/)%2) ;
 ...
 */
   *res_ptr = 0x00 ;
    ...
    */
   for (count=1,ptr=Str_val(string)/*string->value*/; ptr<end_ptr; ptr++, count++)
   {
      if (rx_isspace(*ptr))
      {
         /*
...
//#endif
    ...
    * Note that str_digitize already converts the result
    */
...
streng *std_x2d( tsd_t *TSD, cparamboxptr parms )
{
   ...
   if ((length>0) && (length%2))
   {
      ...
      {
         if (Str_val(packed)/*packed->value*/[msb] & 0x08)
            Str_val(packed)/*packed->value*/[msb] |= 0xf0 ;
         else
            Str_val(packed)/*packed->value*/[msb] &= 0x0f ;
      }
   }

#ifdef REXX_8BIT_SUPPORT
   streng_clone_base_value(packed);
   SET_STRENG_RESTORED(packed->converted);
   SET_STRENG_CONVERTED(packed->converted);
#endif

   /*
    * Note that numerize already convert the result
    */
   result = numerize( TSD,
                      packed,
 ...
 * Note that pack_hex already converts the resuslt
 */
...
streng *std_b2x( tsd_t *TSD, cparamboxptr parms )
{
   ...
   first_group = ptr - Str_val(string)/*string->value*/ ;
    ...
    */
   for (ptr=Str_val(string)/*string->value*/; ptr<endptr; ptr++)
   {
      if (rx_isspace(*ptr))
      {
         /*
...
streng *std_x2b( tsd_t *TSD, cparamboxptr parms )
{
   ...
   ptr = Str_val(parms->value)/*parms->value->value*/ ;
 ...
 * Note that numerize already converts the result
 */
...
#ifdef REXX_8BIT_SUPPORT
   streng_clone_value(parms->value);
   SET_STRENG_RESTORED(parms->value->converted);
   SET_STRENG_CONVERTED(parms->value->converted);
#endif
 ...
 * Note that unpack_hex already converts the result
 */
 ...
 * Note that str_binarize already converts the result
 */
...
streng *std_d2x( tsd_t *TSD, cparamboxptr parms )
{
   ...
   if ( ( length > 0 ) && ( Str_len( result ) != length ) && Str_len( result ) )
   {
      ...
      memmove( Str_val(result)/*result->value*/, &Str_val(result)/*result->value*/[1], --result->len );
   }
    ...
    * the nullstring.
    */
   if ( ( length == -1 ) && ( Str_val(result)/*result->value*/[0] == '0' ) )
   {
      assert( Str_len( result ) > 1 );
      assert( ( Str_val(result)/*result->value*/[0] == '0') && ( ( Str_val(result)/*result->value*/[1] != '0' ) || ( Str_len( result ) == 2 ) ) );
