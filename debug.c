...
void dumpvarcontent( const tsd_t *TSD, FILE *fp, cvariableptr v, int exposed )
{
   ...
   if ( s )
   {
      fprintf( fp, "\"%.*s\"", s->len, Str_val(s)/*s->value*/ );
   }
   else
   {
      fprintf( fp, "<none>" );
...
void dumpvars( const tsd_t *TSD )
{
   ...
   for ( i = 0; i < TSD->currlevel->vars->size; i++ )
   {
      ...
      for ( ptr = hashptr[i]; ptr != NULL; ptr = ptr->next )
      {
         ...
         isstem = Str_val(s)/*s->value*/[s->len - 1] == '.';

         fprintf( fp, "   >>> %3d %s \"%.*s\",\tvalue ",
                      i, ( isstem ) ? "    Stem" : "Variable",
                      s->len, Str_val(s)/*s->value*/ );
...
void dumptree(const tsd_t *TSD, const treenode *thisNode, int level, int newline)
{
   ...
   while ( thisNode ) {
      ...
      if ( thisNode->name )
      {
         fprintf( fp, "%*sName: [%.*s]\n",
                      2 * level, "",
                      thisNode->name->len, Str_val(thisNode->name)/*thisNode->name->value*/ );
      }
      ...
      if ( ( thisNode->charnr != 0 ) && (thisNode->charnr != -1 ) )
      {
         ...
         if ( newline )
         {
           ...
           Str_restore_TSD(ptr);
            ...
            fprintf( fp, ", Sourceline: [%.*s]", ptr->len, Str_val(ptr)/*ptr->value*/ );
         }
         putc( '\n', fp );
      }
...
#ifdef REXX_8BIT_SUPPORT
               streng_alloc_base_value(thisNode->u.strng);
               streng_clone_value(thisNode->u.strng);
               Str_restore_TSD(thisNode->u.strng);
#endif

               fprintf( fp, "%*sValue: [%.*s]\n",
                            2 * level, "",
                            thisNode->u.strng->len, Str_val(thisNode->u.strng)/*thisNode->u.strng->value*/ );
            }
            break;
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(first->line);
   streng_clone_value(first->line);
   Str_restore_TSD(first->line);
#endif
   ...
   for (;first;)
   {
      if (first->lineno==line)
      {
         *size = first->line->len ;
         ...
         return Str_val(first->line)/*first->line->value*/ ;
      }
      else
...
/*   assert( ptr ) ; */
   ...
   outptr = Str_val(string)/*string->value*/ ;
...
endloop:
   assert( outptr - Str_val(string)/*string->value*/ <= BUFFERSIZE ) ;
   *outptr = '\0'; /* needs to be 0-terminated */
   string->len = outptr - Str_val(string)/*string->value*/ ;
