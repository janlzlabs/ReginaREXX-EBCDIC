...
static int get_io_flag( tsd_t *TSD, streng *command, streng **rxqueue )
{
   ...
   {
      if ((!memcmp(Str_val(command)/*command->value*/,"lifo>",5)) ||
          (!memcmp(Str_val(command)/*command->value*/,"LIFO>",5)))
               flag |= REDIR_INPUT ;

      if ((!memcmp(Str_val(command)/*command->value*/+length-5,">lifo",5))
      ||  (!memcmp(Str_val(command)/*command->value*/+length-5,">LIFO",5)))
      {
         flag |= REDIR_OUTLIFO ;
         command->len -= 5;
      }
      ...
      {
         if ( ( !memcmp( Str_val(command)/*command->value*/+length-5, ">fifo" ,5 ) )
         ||   ( !memcmp( Str_val(command)/*command->value*/+length-5, ">FIFO", 5 ) ) )
         {
            flag |= REDIR_OUTFIFO ;
            command->len -= 5;
         }
         else
         {
            if ( length >= 8 )
            {
               ...
               {
                  if ( *(Str_val(command)/*command->value*/ + i ) == '|' )
                  {
                     pos = i;
               ...
               if ( pos != -1 )
               {
                  ...
                  {
                     if ( !rx_isspace(Str_val(command)/*command->value*/[i] ) )
                        break;
                  }
                  if ( i+7 <= length )
                  {
                     if (mem_cmpic( Str_val(command)/*command->value*/ + i, "RXQUEUE", 7 ) == 0 )
                     {
                        i += 7;
                        for ( ; i < length; i++ )
                        {
                        ...
                        else if ( have_space )
                        {
                            if ( *(Str_val(command)/*command->value*/ + i) == '/' )
                            {
                                ...
                                */
                               if ( i+6 <= length
                               &&   mem_cmpic( Str_val(command)/*command->value*/ + i, "/CLEAR", 6 ) == 0 )
                               {
                                  flag |= REDIR_CLEAR;
                                  flag |= REDIR_OUTFIFO ;
                               }
                               ...
                               {
                                  if (mem_cmpic( Str_val(command)/*command->value*/ + i, "/FIFO", 5 ) == 0 )
                                     flag |= REDIR_OUTFIFO ;
                                  else if (mem_cmpic( Str_val(command)/*command->value*/ + i, "/LIFO", 5 ) == 0 )
                                     flag |= REDIR_OUTLIFO ;
                                  else
                                     flag |= REDIR_OUTFIFO ;
                                  /*
                            ...
                            else
                            {
...
#ifdef REXX_8BIT_SUPPORT
                                     streng_alloc_base_value(tmpq);
                                     streng_clone_value(tmpq);
#endif

                                     tmpq->len = qname_end - qname_start;
                                     memcpy( Str_val(tmpq)/*tmpq->value*/, Str_val(command)/*command->value*/ + qname_start, qname_end - qname_start );
                                     break;
                                  }
                               }
                               if ( have_space )
                               ...
                               else
                               {
                                  ...
                                  memcpy( Str_val(tmpq)/*tmpq->value*/, Str_val(command)/*command->value*/ + qname_start, length - qname_start );
                                  flag |= REDIR_OUTFIFO ;
                               }
                            }
                            command->len = pos;
   ...
   if (flag & REDIR_INPUT)
   {
      for(i=5; i<Str_len(command); i++ ) /* avoid buffer overrun */
         Str_val(command)/*command->value*/[i-5]=Str_val(command)/*command->value*/[i] ;
      command->len -= 5 ;
   }

#ifdef REXX_8BIT_SUPPORT
   streng_clone_base_value(command);
#endif

   Str_convert_TSD(command);
...
streng *run_popen( tsd_t *TSD, const streng *command, const streng *envir )
{
   ...
   if ( !Str_len(retval) ) {
       if ( ((environment *)TSD->firstenvir)->arg )
           retval = (streng *)((environment *)TSD->firstenvir)->arg;
   }
