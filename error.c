...
static const errtext_t errtext[NUMBER_ERROR_MESSAGES] =
{
   ...
   {  94, 99,"[Internal error with external queue interface: %d \"%s\"]|<errcode>,<systemerror>" },
...
#define ERR1PREFIX_IDX   0
#define SUBERRPREFIX_IDX 1
#define ERR2PREFIX_IDX   2
static const char *err1prefix = "Error %d running \"%.*s\", line %d: %.*s";
static const char *suberrprefix = "Error %d.%d: %.*s";
static const char *err2prefix = "Error %d running \"%.*s\": %.*s";

static const char *erropen = "Unable to open language file: %s";
static const char *errcount = "Incorrect number of messages in language file: %s";
static const char *errread = "Unable to read from language file: %s";
static const char *errmissing = "Text missing from language file: %s.mtb";
static const char *errcorrupt = "Language file: %s.mtb is corrupt";
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(et->errornum);
   streng_clone_value(et->errornum);
#endif
...
static streng *get_buffer( const tsd_t *TSD, unsigned minsize )
{
...
#ifdef REXX_8BIT_SUPPORT
         streng_alloc_base_value(et->buffer[idx]);
         streng_clone_value(et->buffer[idx]);
#endif
   ...
   return Str_convert_TSD(et->buffer[idx]);
}
                           ...
                           const char *arg )
{
   ...
   Str_len( retval ) = sprintf( Str_val(retval)/*retval->value*/, fmt, arg );
                                 ...
                                 int errorno, int suberrorno, int *is_fmt )
{
   ...
   if ( fread( Str_val(retval)/*retval->value*/, 1, textlength, fp ) != textlength )
   {
      *is_fmt = 0;
      return simple_msg( TSD, errcorrupt, errfn );
   }
   Str_val(retval)/*retval->value*/[textlength] = '\0';
...
void exiterror( int errorno, int suberrorno, ... )
{
     ...
     || ( ( errorno == ERR_STORAGE_EXHAUSTED ) && ( et->conditions > 10 ) ) )
   {
      ...
      if ( regina_output_conv_handler_get() ) {
          regina_output_conv_handler_get()(out, len);
      }
   ...
   //printf("line_num %p %p line_num(%d)\n", user_handlers.GetErrorLineNumHandler, user_handlers.GetErrorLineNumArg, lineno);
   if ( user_handlers.GetErrorLineNumHandler ) {
       user_handlers.GetErrorLineNumHandler(user_handlers.GetErrorLineNumArg, (unsigned int)lineno);
   }
   ...
   if ( ( errorno <= ERR_MAX_NUMBER ) && ( suberrorno != 0 ) )
   {
      ...
      len = sprintf( Str_val(errmsg)/*errmsg->value*/, Str_val(ptr)/*ptr->value*/,
                     errorno, suberrorno, Str_len( fmt ), Str_val(fmt)/*fmt->value*/ );

      va_start( argptr, suberrorno );
      for ( i = 0; i < Str_len( fmt ); i++ )
      {
         if ( Str_val(fmt)/*->value*/[i] == '%' )
         {
            switch ( Str_val(fmt)/*fmt->value*/[i+1] )
            {
               case 's':
...
#ifdef REXX_8BIT_SUPPORT
      streng_alloc_base_value(suberror_streng);
      streng_clone_value(suberror_streng);
#endif

      if ( suberror_streng )
      {
         va_start( argptr, suberrorno );
         Str_len( suberror_streng ) = vsprintf( Str_val(suberror_streng)/*suberror_streng->value*/,
                                                Str_val(errmsg)/*errmsg->value*/, argptr );
         va_end( argptr );
      }
   }
#endif
   ...
   //printf("file_name %p %p file_name(%s)\n", user_handlers.GetErrorFileNameHandler, user_handlers.GetErrorFileNameArg,
       //!inputfile ? "file unknown" : Str_val(inputfile));
   if ( user_handlers.GetErrorFileNameHandler ) {
       user_handlers.GetErrorFileNameHandler(user_handlers.GetErrorFileNameArg, Str_val(inputfile));
   }
...
#ifdef REXX_8BIT_SUPPORT
      streng_alloc_base_value(errmsg);
      streng_clone_value(errmsg);
#endif

      sprintf( Str_val(errmsg)/*errmsg->value*/, Str_val(ptr)/*ptr->value*/,
               errorno, Str_len( inputfile ), Str_val(inputfile)/*inputfile->value*/, lineno,
               Str_len( etext ), Str_val(etext)/*etext->value*/ );
   }
   else
   {
      if ( et->native_language == LANGUAGE_ENGLISH
      ...
      sprintf( Str_val(errmsg)/*errmsg->value*/, Str_val(ptr)/*ptr->value*/,
               errorno, Str_len( inputfile ), Str_val(inputfile)/*inputfile->value*/,
               Str_len( etext ), Str_val(etext)/*etext->value*/ );
   }

   errmsg->len = strlen( Str_val(errmsg)/*errmsg->value*/ );
...
#ifdef REXX_8BIT_SUPPORT
      streng_clone_base_value(errmsg);
#endif
      Str_convert_TSD(errmsg);

      if ( !yyout_io )
          fwrite( Str_origval(errmsg)/*errmsg->value*/, Str_len(errmsg), 1, fp );
      else
          FWRITE( Str_origval(errmsg)/*errmsg->value*/, Str_len(errmsg), 1, yyout_io );
#if defined(DOS) || defined(OS2) || defined(WIN32)
      /*
       * stdout is open in binary mode, so we need to add the
       * extra CR to the end of the line.
   ...
   if ( ( errorno <= ERR_MAX_NUMBER ) && suberrorno && suberror_streng )
   {
      ...
      {
#ifdef REXX_8BIT_SUPPORT
          streng_clone_base_value(suberror_streng);
#endif
          Str_convert_TSD(suberror_streng);

         if ( !yyout_io )
             fwrite( Str_origval(suberror_streng)/*suberror_streng->value*/, Str_len(suberror_streng), 1, fp );
         else
             FWRITE( Str_origval(suberror_streng)/*suberror_streng->value*/, Str_len(suberror_streng), 1, yyout_io );
#if defined(DOS) || defined(OS2) || defined(WIN32)
         /*
          * stdout is open in binary mode, so we need to add the
          * extra CR to the end of the line.
...
not_hookable:
   ...
   if ( TSD->systeminfo->script_exit )
   {
      ...
      if ( et != NULL )
      {
          ...
          */
         et->errornum->len = sprintf( Str_val(et->errornum)/*et->errornum->value*/, "%d", errorno*1000 + suberrorno );
#ifdef REXX_8BIT_SUPPORT
         streng_alloc_base_value(et->errornum);
         streng_clone_base_value(et->errornum);
         SET_STRENG_RESTORED(et->errornum->converted);
         SET_STRENG_CONVERTED(et->errornum->converted);
#endif

         jump_script_exit( TSD, et->errornum );

         /*
          * This may seem strange but we don't make a jump if in interactive mode,
          * we continue taking input
          */
         return;
      }
   }
                                ...
                                struct textindex *tpi )
{
    ...
    * NUMBER_ERROR_MESSAGES in rexxmsg.h
    */
    ...
    * NUMBER_PREFIX_MESSAGES in rexxmsg.h
    */
   if ( et->number_prefix_messages != NUMBER_PREFIX_MESSAGES )
   {
      fclose( *fp );
...
static streng *get_message_indexes( const tsd_t *TSD )
{
...
#if defined(__EPOC32__) || defined(__WINS__)
   FILE *fp;
   int number_messages, file_lang. number_prefix_messages;
   struct stat buffer ;
#else
   int i, found=0;
#endif
   ...
   else
   {
      ...
      if ( !found )
      {
         ...
         Str_len( err ) = sprintf( Str_val(err)/*err->value*/, "Unsupported native language \"%.*s\"",
                                               len, ptr );
         return Str_convert_TSD(err);
      }
   }
   if ( et->native_language != LANGUAGE_ENGLISH )
   {
...
# endif
         ...
         return Str_convert_TSD(simple_msg( TSD, "Length of \"%s\" exceeds the path's maximum", errpath ));
   }
#endif
   /*
...
const streng *errortext( const tsd_t *TSD, int errorno, int suberrorno, int request_english, int apply_inserts )
{
   ...
   if (errorno>100)
   {
      return Str_convert_TSD(simple_msg( TSD, "%s", strerror(errorno-100) ));
   }
   ...
   {
      if ( Str_val(ptr)/*ptr->value*/[low] == '|' )
      {
         Str_val(ptr)/*ptr->value*/[low] = '\0';
         Str_len( ptr ) = low;
         have_inserts = 1;
         break;
      }
   ...
   &&   have_inserts )
   {
      ...
      {
         if ( Str_val(ptr)/*ptr->value*/[end] == '%' )
         {
            switch( Str_val(ptr)/*ptr->value*/[end+1] ) /* assumes no message ends in % */
            {
               case 's':
                  num_inserts++;
                  break;
      ...
      ins = Str_val(ptr)/*ptr->value*/+low+1;
      insert[0] = ins;
      low = strlen( ins );
      ...
      {
         case 1:
            Str_len( h ) = sprintf( Str_val(h)/*h->value*/, Str_val(ptr)/*ptr->value*/, insert[0] );
            break;
         case 2:
            Str_len( h ) = sprintf( Str_val(h)/*h->value*/, Str_val(ptr)/*ptr->value*/, insert[0], insert[1] );
            break;
         case 3:
            Str_len( h ) = sprintf( Str_val(h)/*h->value*/, Str_val(ptr)/*ptr->value*/, insert[0], insert[1], insert[2] );
            break;
         case 4:
            Str_len( h ) = sprintf( Str_val(h)/*h->value*/, Str_val(ptr)/*ptr->value*/, insert[0], insert[1], insert[2], insert[3] );
            break;
         case 5:
            Str_len( h ) = sprintf( Str_val(h)/*h->value*/, Str_val(ptr)/*ptr->value*/, insert[0], insert[1], insert[2], insert[3], insert[4] );
            break;
      }
      ptr = h;
   }
...
const char *getsym( int numb )
{
   ...
   switch (numb)
   {
      ...
      case X_PARSE_NUM: symb="Parse numeric" ; break ;
