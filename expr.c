...
static int str_to_bool( const streng *input )
{
   ...
   ch = Str_val(input)/*input->value*/[0] ;
   if (ch!='0' && ch!='1')
       exiterror( ERR_UNLOGICAL_VALUE, 0 )  ;
...
#ifdef REXXDEBUG
   fprintf(stderr, "func=%s thisptr->type=%d\n", __FUNCTION__, thisptr ? thisptr->type : -100);
#endif

   /*
    * To prevent a syntax error to cause a crash we need to return a bogus num value.
    */
   if ( !thisptr ) {
       return bool_to_num(TSD, 0);
   }
...
#define RETURN_NEW(val) if ( kill )     \
   ...
   strone = strtwo = strthr = NULL;
   stmp1 = stmp2 = NULL;
   cstmp = NULL;

#ifdef REXXDEBUG
   fprintf(stderr, "func=%s thisptr->type=%s\n", __FUNCTION__, thisptr ? get_rexx_type_text(thisptr->type) : -100);
#endif
   ...
   /*
    * To prevent a syntax error to cause a crash we need to return a bogus value.
    */
   if ( !thisptr ) {
       RETURN_NEW( Str_cre_TSD(TSD, "") );
   }
   ...
   switch ( thisptr->type )
   {
      ...
      case X_CON_SYMBOL:
         ...
         if ( kill )
            return Str_convert_TSD((streng *) cstmp); /* and *kill is set to NULL above */
      ...
      case X_IS_INTERNAL:
      {
         ...
         if ( get_options_flag( TSD->currlevel, EXT_STRICT_ANSI ) )
         {
            if ( thisptr->name->value[(thisptr->name->len)-1] == Char_convert('.') )
...
#ifdef REXX_8BIT_SUPPORT
         streng_alloc_base_value(strthr);
         streng_clone_value(strthr);

         streng_alloc_base_value(strtwo);
         streng_clone_value(strtwo);
         Str_restore_TSD(strtwo);

         streng_alloc_base_value(strone);
         streng_clone_value(strone);
         Str_restore_TSD(strone);
#endif

         cptr = Str_val(strthr); /* strthr->value ; */
         memcpy( cptr, Str_val(strone) /* strone->value */, strone->len ) ;
         cptr += strone->len ;
         if (thisptr->type==X_SPACE)
            *(cptr++) = ' ' ;

         memcpy( cptr, Str_val(strtwo) /* strtwo->value */, strtwo->len ) ;
         strthr->len = (cptr-Str_val(strthr)) /* strthr->value) */ + strtwo->len ;

#ifdef REXX_8BIT_SUPPORT
         streng_clone_base_value(strthr);
         Str_convert_TSD(strthr);
#endif

#ifdef REXXDEBUG
         fprintf(stderr, "\nstrtwo ");
         print_stre(stderr, "ASCII", strtwo);
         fprintf(stderr, "\nstrone ");
         print_stre(stderr, "ASCII", strone);
         fprintf(stderr, "\n");
#endif
...
#ifdef REXXDEBUG
            fprintf(stderr, "\nstrthr trace(I)");
            print_stre(stderr, "ASCII", strthr);
            fprintf(stderr, "\n");
#endif
   ...
   switch ( thisptr->type )
   {
      ...
      case X_LTE:
      {
         ...
         else
         {
            ...
            if ( lnum ) {
                s1 = (unsigned char *) Str_origval(lval)/*lval->value*/; /* base_value for stripping white spaces */
            } else {
                s1 = (unsigned char *) Str_val(lval)/*lval->value*/; /* base_value for stripping white spaces */
            }
            if ( rnum ) {
                s2 = (unsigned char *) Str_origval(rval)/*rval->value*/; /* base_value for stripping white spaces */
            } else {
                s2 = (unsigned char *) Str_val(rval)/*rval->value*/; /* base_value for stripping white spaces */
            }
...
#ifdef REXX_8BIT_SUPPORT
            streng_clone_base_value(lval);
            Str_convert_TSD(lval);
            streng_clone_base_value(rval);
            Str_convert_TSD(rval);
#endif
      ...
      {  /* string comparison */
         ...
         s1 = (unsigned char *) Str_origval(strone)/*strone->value*/ ; /* base_value for stripping white spaces */
         s2 = (unsigned char *) Str_origval(strtwo)/*strtwo->value*/ ; /* base_value for stripping white spaces */
...
#ifdef REXX_8BIT_SUPPORT
         streng_clone_base_value(strone);
         Str_convert_TSD(strone);
         streng_clone_base_value(strtwo);
         Str_convert_TSD(strtwo);
#endif
      ...
      {  /* strict string NOT comparison */
         ...
         s1 = (unsigned char *) Str_val(strone)/*strone->value*/ ; /* base_value for stripping white spaces */
         s2 = (unsigned char *) Str_val(strtwo)/*strtwo->value*/ ; /* base_value for stripping white spaces */
         e1 = (unsigned char *) s1 + strone->len ;
         e2 = (unsigned char *) s2 + strtwo->len ;
         /*
