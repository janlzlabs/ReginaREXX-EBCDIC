...
#ifdef REXX_8BIT_SUPPORT
int fstat_8bit(int fd, struct stat *buf)
{
    return fstat(fd, buf);
}

int fseek_8bit(FILE *stream, long offset, int whence)
{
    return fseek(stream, offset, whence);
}

long ftell_8bit(FILE *stream)
{
    return ftell(stream);
}

off_t ftello_8bit(FILE *stream)
{
    if ( regina_io_handlers_initialized_get() && stream != stdin ) {
        return 0;
    }
...
{
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(cmd);
   streng_clone_value(cmd);
   Str_restore_TSD(cmd);
#endif

   Str_upper(cmd);

   if (cmd->len==4 && !memcmp(Str_val(cmd)/*cmd->value*/, "READ", 4))
      return COMMAND_READ ;
   if (cmd->len==5 && !memcmp(Str_val(cmd)/*cmd->value*/, "WRITE", 5))
      return COMMAND_WRITE ;
   if (cmd->len==6 && !memcmp(Str_val(cmd)/*cmd->value*/, "APPEND", 6))
      return COMMAND_APPEND ;
   if (cmd->len==6 && !memcmp(Str_val(cmd)/*cmd->value*/, "UPDATE", 6))
      return COMMAND_UPDATE ;
   if (cmd->len==6 && !memcmp(Str_val(cmd)/*cmd->value*/, "CREATE", 6))
      return COMMAND_CREATE ;
   if (cmd->len==5 && !memcmp(Str_val(cmd)/*cmd->value*/, "CLOSE", 5))
      return COMMAND_CLOSE ;
   if (cmd->len==5 && !memcmp(Str_val(cmd)/*cmd->value*/, "FLUSH", 5))
      return COMMAND_FLUSH ;
   if (cmd->len==6 && !memcmp(Str_val(cmd)/*cmd->value*/, "STATUS", 6))
      return COMMAND_STATUS ;
   if (cmd->len==5 && !memcmp(Str_val(cmd)/*cmd->value*/, "FSTAT", 5))
      return COMMAND_FSTAT ;
   if (cmd->len==5 && !memcmp(Str_val(cmd)/*cmd->value*/, "RESET", 5))
      return COMMAND_RESET ;
   if (cmd->len==8 && !memcmp(Str_val(cmd)/*cmd->value*/, "READABLE", 8))
      return COMMAND_READABLE ;
   if (cmd->len==8 && !memcmp(Str_val(cmd)/*cmd->value*/, "WRITABLE", 8))
      return COMMAND_WRITEABLE ;
   if (cmd->len==10 && !memcmp(Str_val(cmd)/*cmd->value*/, "EXECUTABLE", 10))
      return COMMAND_EXECUTABLE ;
   if (cmd->len==4 && !memcmp(Str_val(cmd)/*cmd->value*/,  "LIST", 4))
      return COMMAND_LIST ;
   if (cmd->len>=4 && !memcmp(Str_val(cmd)/*cmd->value*/,  "OPEN", 4))
      return COMMAND_OPEN ;
   if (cmd->len>=5 && !memcmp(Str_val(cmd)/*cmd->value*/,  "QUERY", 5))
      return COMMAND_QUERY ;
   if (cmd->len>=4 && !memcmp(Str_val(cmd)/*cmd->value*/,  "SEEK", 4))
      return COMMAND_SEEK ;
   if (cmd->len>=8 && !memcmp(Str_val(cmd)/*cmd->value*/,  "POSITION", 8))
...
static char get_querycommand( const streng *cmd )
{
   ...
   if (cmd->len==8 && !memcmp(Str_val(cmd)/*cmd->value*/,  "DATETIME", 8))
      return COMMAND_QUERY_DATETIME ;
   if (cmd->len==6 && !memcmp(Str_val(cmd)/*cmd->value*/,  "EXISTS", 6))
      return COMMAND_QUERY_EXISTS ;
   if (cmd->len==6 && !memcmp(Str_val(cmd)/*cmd->value*/,  "HANDLE", 6))
      return COMMAND_QUERY_HANDLE ;
   if (cmd->len>=4 && !memcmp(Str_val(cmd)/*cmd->value*/,  "SEEK", 4))
      return COMMAND_QUERY_SEEK ;
   if (cmd->len>=8 && !memcmp(Str_val(cmd)/*cmd->value*/,  "POSITION", 8))
      return COMMAND_QUERY_POSITION ;
   if (cmd->len==4  && !memcmp(Str_val(cmd)/*cmd->value*/, "SIZE", 4))
      return COMMAND_QUERY_SIZE ;
   if (cmd->len==10 && !memcmp(Str_val(cmd)/*cmd->value*/, "STREAMTYPE", 10))
      return COMMAND_QUERY_STREAMTYPE ;
   if (cmd->len==9  && !memcmp(Str_val(cmd)/*cmd->value*/, "TIMESTAMP", 9))
      return COMMAND_QUERY_TIMESTAMP ;
   if (cmd->len==10  && !memcmp(Str_val(cmd)/*cmd->value*/, "CREATETIME", 10))
      return COMMAND_QUERY_CREATETIME;
   if (cmd->len==10  && !memcmp(Str_val(cmd)/*cmd->value*/, "MODIFYTIME", 10))
      return COMMAND_QUERY_MODIFYTIME;
   if (cmd->len==10  && !memcmp(Str_val(cmd)/*cmd->value*/, "ACCESSTIME", 10))
...
static char get_querypositioncommand( const streng *cmd )
{
   ...
   if (cmd->len==3 && !memcmp(Str_val(cmd)/*cmd->value*/,  "SYS", 3))
...
static char get_querypositionreadcommand( const streng *cmd )
{
   ...
   if (cmd->len==4 && !memcmp(Str_val(cmd)/*cmd->value*/,  "CHAR", 4))
      return COMMAND_QUERY_POSITION_READ_CHAR ;
   if (cmd->len==4 && !memcmp(Str_val(cmd)/*cmd->value*/,  "LINE", 4))
...
static char get_opencommand( const streng *cmd )
{
   ...
   if (cmd->len>=4 && !memcmp(Str_val(cmd)/*cmd->value*/,  "BOTH", 4))
...
static char get_opencommandboth( const streng *cmd )
{
   ...
   if (cmd->len==7 && !memcmp(Str_val(cmd)/*cmd->value*/,  "REPLACE", 7))
...
#define filehashvalue(strng) (hashvalue(Str_val(strng)/*strng->value*/, strng->len) % FILEHASH_SIZE)
#else
#define filehashvalue(strng) (hashvalue_ic(Str_val(strng)/*strng->value*/, strng->len) % FILEHASH_SIZE)
#endif
...
tryagain:
   ...
   errno = 0 ;
#endif
   ptr->fileptr = fopen( Str_val(ptr->filename0)/*ptr->filename0->value*/, acc_mode[faccess] ) ;
...
{
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(name1);
   streng_clone_value(name1);
   Str_restore_TSD(name1);

   streng_alloc_base_value(name2);
   streng_clone_value(name2);
   Str_restore_TSD(name2);
#endif
...
int init_filetable( tsd_t *TSD )
{
   ...
   if ( !io_handlers.inputGetcHandler || io_handlers.inputGetcHandler == getc ) {
       ft->stdio_ptr[0]->fileptr = ft->stdio_ptr[3]->fileptr = stdin ;
   } else {
       ft->stdio_ptr[0]->fileptr = ft->stdio_ptr[3]->fileptr = yyin_io;
   }
...
void fixup_file( tsd_t *TSD, const streng *filename )
{
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(filename);
   streng_clone_value(filename);
   Str_restore_TSD(filename);
#endif
   ...
   if ( filename )
   {
      ...
      if (ptr)
      {
         ...
         if (ptr->fileptr)
         {
            if ( regina_clear_error_handler_get() ) {
               regina_clear_error_handler_get()(io_handlers.clearFileErrorHandlerArg, ptr->fileptr);
            } else {
...
static void reopen_file( tsd_t *TSD, fileboxptr ptr )
{
   ...
   fclose( ptr->fileptr ) ;
#ifdef VMS
   ptr->fileptr = fopen( Str_val(ptr->filename0)/*ptr->filename0->value*/, "r+" ) ;
#else
   ptr->fileptr = fopen( Str_val(ptr->filename0)/*ptr->filename0->value*/, "r+b" ) ;
#endif
   if (ptr->fileptr==NULL)
   {
      file_error( ptr, errno, NULL ) ;
...
try_to_open:
   ...
   {
#ifdef VMS
      if ((ptr->fileptr = fopen( Str_val(name)/*name->value*/, "r" )) != NULL)
#else
      if ((ptr->fileptr = fopen( Str_val(name)/*name->value*/, "rb" )) != NULL)
#endif
      {
   ...
   else if (faccess==ACCESS_WRITE)
   {
      ...
      ptr->flag = FLAG_READ ;
#ifdef VMS
      ptr->fileptr = fopen( Str_val(name)/*name->value*/, "r+" ) ;
#else
      ptr->fileptr = fopen( Str_val(name)/*name->value*/, "r+b" ) ;
#endif
      errno = 0 ;
      if (!ptr->fileptr)
#ifdef VMS
         ptr->fileptr = fopen( Str_val(name)/*name->value*/, "w+" ) ;
#else
         ptr->fileptr = fopen( Str_val(name)/*name->value*/, "w+b" ) ;
#endif
      ...
      if (!ptr->fileptr)
      {
         ...
         ptr->fileptr = fopen( Str_val(name)/*name->value*/, "w" ) ;
#else
         ptr->fileptr = fopen( Str_val(name)/*name->value*/, "wb" ) ;
#endif
         ptr->flag &= 0 ;
      }
   ...
   else if (faccess==ACCESS_APPEND)
   {
       ...
       */
#ifdef VMS
      if ((ptr->fileptr = fopen( Str_val(name)/*name->value*/, "a" )) != NULL)
#else
      if ((ptr->fileptr = fopen( Str_val(name)/*name->value*/, "ab" )) != NULL)
#endif
      {
   ...
   else if (faccess==ACCESS_STREAM_REPLACE)
   {
       ...
       */
#ifdef VMS
      if ((ptr->fileptr = fopen( Str_val(name)/*name->value*/, "w+" )) != NULL)
#else
      if ((ptr->fileptr = fopen( Str_val(name)/*name->value*/, "w+b" )) != NULL)
#endif
      {
   ...
   else if (faccess==ACCESS_UPDATE)
   {
      ...
      if ((ptr->fileptr = fopen( Str_val(name)/*name->value*/, "r+" )) != NULL)
#else
      if ((ptr->fileptr = fopen( Str_val(name)/*name->value*/, "r+b" )) != NULL)
#endif
      {
...
/*
 * This function will display a prompt, if in interactive mode.
 * Prompt will get displayed after conversion, i.e. when Regina is run
 * with EBCDIC processing it'll be converted to EBCDIC and then send
 * omn yyout_io stream.
 */
extern FILE *yyout_io;
static void putprompt()
{
    if ( regina_put_prompt_handler_get() ) {
        char conv_prompt[32];
        const char *prompt = regina_put_prompt_handler_get()(io_handlers.outputPutPromptHandlerArg);
        int prompt_len = strlen(prompt);

        strncpy(conv_prompt, prompt, sizeof(conv_prompt));
        if ( regina_output_conv_handler_get() ) {
            regina_output_conv_handler_get()(conv_prompt, prompt_len);
        }

        FWRITE(conv_prompt, prompt_len, 1, yyout_io);
        FFLUSH(yyout_io);
    }
}
...
/*#endif*/
...
#ifdef REXX_8BIT_SUPPORT
      return Str_convert_TSD(streng_clone_value(streng_alloc_base_value(nullstringptr())));
#else
   ...
   /*
    * This extend the functionality with a prompt feature - only in interactive mode.
    * When a prompt callback is defined we put out a prompt on yyout_io stream.
    */
   if ( TSD->interactive && !TSD->wait_input && yyout_io ) {
       putprompt();
   }
  ...
  */
   ...
   for (i=0; ; i++)
   {
      if ( regina_io_handlers_initialized_get() ) {
         j = io_handlers.inputGetcHandler(ptr->fileptr, 0, 0);
      } else {
         j = getc(ptr->fileptr);
      }
...
/*#if !defined(UNIX) && !defined(MAC)*/
      ...
      {
         if ( regina_io_handlers_initialized_get() ) {
            k = io_handlers.inputGetcHandler(ptr->fileptr, 0, 0);
         } else {
            k = getc(ptr->fileptr);
         }
...
/*            file_warning( ptr, 0, "EOF on line input" ) ; */
   ...
   /*
    * If all we got was newline, we need to pass at leat one space further
    */
   if ( eolf==REGINA_EOL && !i ) {
       ft->rol_string[i++] = ' ';
   }

   if ( ptr->flag & FLAG_RDEOF && TSD->interactive ) {
       strcpy(&ft->rol_string[i], "exit\n");
       i += strlen("exit\n");
   }
...
/*   if (i>1000) i = 1000 ; */
   ...
   memcpy( Str_val(ret)/*ret->value*/, ft->rol_string, ret->len=i ) ;

#ifdef REXX_8BIT_SUPPORT
   streng_clone_base_value(ret);
#endif

   if ( TSD->interactive && TSD->wait_input ) {
       TSD->wait_input = 0;
   }
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(retvalue);
   streng_clone_value(retvalue);
#endif

   errno = 0 ;
   didread = fread( Str_val(retvalue)/*retvalue->value*/, 1, length, fileptr->fileptr ) ;
...
#ifdef REXX_8BIT_SUPPORT
   streng_clone_base_value(retvalue);
#endif

   return Str_convert_TSD(retvalue) ;
}
...
static size_t writebytes( tsd_t *TSD, fileboxptr fileptr, const streng *string, int noerrors )
{
    ...
    */
   errno = 0 ;
   buf = Str_val(string)/*string->value*/ ;
   todo = string->len ;
...
static int writeoneline( tsd_t *TSD, fileboxptr ptr, const streng *data )
{
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(data);
   streng_clone_value(data);
#endif
   ...
   int jj = 0;                                          // LZLANG-759
   for (i=data->value; jj < data->len; i++, ++jj)       // LZLANG-759
// for (i=data->value; i<Str_end(data); i++)            // LZLANG-759
   {
...
static streng *getstatus( tsd_t *TSD, const streng *filename , int subcommand )
{
   ...
   switch ( subcommand )
   {
      case COMMAND_FSTAT:
         ...
         sprintf( Str_val(result)/*result->value*/,
          ...
          */
         strcat( Str_val(result)/*result->value*/, stream_types[determine_stream_type( buffer.st_mode )].streamname );
         break;
      case COMMAND_QUERY_EXISTS:
         if ( streamtype == STREAMTYPE_TRANSIENT )
         {
#ifdef REXX_8BIT_SUPPORT
            result = Str_convert_TSD(streng_clone_value(streng_alloc_base_value(nullstringptr())));
#else
         ...
         else
         {
            ...
            my_fullpath( TSD, Str_val(result)/*result->value*/, fn );
            result->len = strlen( Str_val(result)/*result->value*/ );
         }
         break;
...
#if defined(HAVE_I64U)
            sprintf( Str_val(result)/*result->value*/, "%I64u", buffer.st_size ) ;
#else
            if ( sizeof(rx_64) > 4 )
               sprintf( Str_val(result)/*result->value*/, "%lld", (rx_64)(buffer.st_size) ) ;
            else
               sprintf( Str_val(result)/*result->value*/, "%ld", (long)(buffer.st_size) ) ;
#endif
         }
         break;
      case COMMAND_QUERY_HANDLE:
         if (fno)
         {
            ...
            sprintf( Str_val(result)/*result->value*/, "%d", fno ) ;
         }
         else
#ifdef REXX_8BIT_SUPPORT
            result = Str_convert_TSD(streng_clone_value(streng_alloc_base_value(nullstringptr())));
      ...
      case COMMAND_QUERY_STREAMTYPE:
         ...
         sprintf( Str_val(result)/*result->value*/, "%s", streamdesc[streamtype] ) ;
         break;
...
#if 0
            sprintf( Str_val(result)/*result->value*/, fmt, tmdata.tm_mon+1, tmdata.tm_mday,
                  (tmdata.tm_year % 100), tmdata.tm_hour, tmdata.tm_min,
                  tmdata.tm_sec ) ;
#else
            strftime( Str_val(result)/*result->value*/, 20, "%m-%d-%y %H:%M:%S", &tmdata );
#endif
         }
         break;
...
#if 0
            sprintf( Str_val(result)/*result->value*/, iso, tmdata.tm_year+1900, tmdata.tm_mon+1,
                  tmdata.tm_mday,
                  tmdata.tm_hour, tmdata.tm_min,
                  tmdata.tm_sec ) ;
#else
            strftime( Str_val(result)/*result->value*/, 20, "%Y-%m-%d %H:%M:%S", &tmdata );
#endif
         }
         break;
...
#if defined(HAVE_I64U)
            sprintf( Str_val(result)/*result->value*/, "%I64u", buffer.st_ctime );
#else
            if ( sizeof(off_t) > 4 )
               sprintf( Str_val(result)/*result->value*/, "%lld", buffer.st_ctime );
            else
               sprintf( Str_val(result)/*result->value*/, "%ld", buffer.st_ctime);
#endif
         }
         break;
...
#if defined(HAVE_I64U)
            sprintf( Str_val(result)/*result->value*/, "%I64u", buffer.st_mtime );
#else
            if ( sizeof(off_t) > 4 )
               sprintf( Str_val(result)/*result->value*/, "%lld", buffer.st_mtime );
            else
               sprintf( Str_val(result)/*result->value*/, "%ld", buffer.st_mtime);
#endif
         }
         break;
...
#if defined(HAVE_I64U)
            sprintf( Str_val(result)/*result->value*/, "%I64u", buffer.st_atime );
#else
            if ( sizeof(off_t) > 4 )
               sprintf( Str_val(result)/*result->value*/, "%lld", buffer.st_atime );
            else
               sprintf( Str_val(result)/*result->value*/, "%ld", buffer.st_atime);
#endif
         }
         break;
...
#if defined(HAVE_I64U)
            sprintf( Str_val(result)/*result->value*/, "%I64u", pos_read + 1 );
#else
            if ( sizeof(off_t) > 4 )
               sprintf( Str_val(result)/*result->value*/, "%lld", pos_read + 1 );
            else
               sprintf( Str_val(result)/*result->value*/, "%ld", (long)(pos_read + 1) );
#endif
         }
         else
#ifdef REXX_8BIT_SUPPORT
...
#if defined(HAVE_I64U)
            sprintf( Str_val(result)/*result->value*/, "%I64u", pos_write + 1 );
#else
            if ( sizeof(off_t) > 4 )
               sprintf( Str_val(result)/*result->value*/, "%lld", pos_write + 1 );
            else
               sprintf( Str_val(result)/*result->value*/, "%ld", (long)(pos_write + 1) );
#endif
         }
         else
#ifdef REXX_8BIT_SUPPORT
...
#if defined(HAVE_I64U)
            sprintf( Str_val(result)/*result->value*/, "%I64u", line_read );
#else
            if ( sizeof(off_t) > 4 )
               sprintf( Str_val(result)/*result->value*/, "%lld", line_read );
            else
               sprintf( Str_val(result)/*result->value*/, "%ld", (long)line_read );
#endif
         }
         else
#ifdef REXX_8BIT_SUPPORT
...
#if defined(HAVE_I64U)
            sprintf( Str_val(result)/*result->value*/, "%I64u", line_write + 1 );
#else
            if ( sizeof(off_t) > 4 )
               sprintf( Str_val(result)/*result->value*/, "%lld", line_write + 1 ) ;
            else
               sprintf( Str_val(result)/*result->value*/, "%ld", (long)(line_write + 1) ) ;
#endif
...
#if defined(HAVE_I64U)
            sprintf( Str_val(result)/*result->value*/, "%I64u", line_write );
#else
            if ( sizeof(off_t) > 4 )
               sprintf( Str_val(result)/*result->value*/, "%lld", line_write ) ;
            else
               sprintf( Str_val(result)/*result->value*/, "%ld", (long)line_write ) ;
#endif
         }
         else
#ifdef REXX_8BIT_SUPPORT
...
static streng *getrexxstatus( const tsd_t *TSD, cfileboxptr ptr )
{
   ...
   Str_val(result)/*result->value*/[0] = 0x00 ;

   if ((ptr->flag & FLAG_READ) && (ptr->flag & FLAG_WRITE))
      strcat( Str_val(result)/*result->value*/, "READ/WRITE" ) ;
   else if (ptr->flag & FLAG_READ)
      strcat( Str_val(result)/*result->value*/, "READ" ) ;
   else if (ptr->flag & FLAG_WRITE)
      strcat( Str_val(result)/*result->value*/, "WRITE" ) ;
   else
      strcat( Str_val(result)/*result->value*/, "NONE" ) ;

#if defined(HAVE_I64U)
   sprintf( Str_val(result)/*result->value*/ + strlen(Str_val(result)/*result->value*/),
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(subcommand);
   streng_clone_value(subcommand);
   Str_restore_TSD(subcommand);
...
static streng *getopen( tsd_t *TSD, const streng *filename , const streng *subcommand)
{
   ...
   switch ( oper )
   {
      case COMMAND_OPEN_BOTH :
         if ( subcommand->len >= 4
         &&   memcmp(Str_val(subcommand)/*subcommand->value*/, "BOTH", 4) == 0 )
            psub = Str_nodupTSD( subcommand, 4, subcommand->len - 4 );
         else
...
#if defined(HAVE_I64U)
      sprintf(Str_val(result)/*result->value*/, "%I64u", pos );
#else
      if ( sizeof(rx_64) > 4 )
         sprintf(Str_val(result)/*result->value*/, "%lld", pos );
      else
         sprintf(Str_val(result)/*result->value*/, "%ld", (long)pos );
#endif
      Str_len( result ) = strlen( Str_val(result)/*result->value*/ );
   }
   else
   {
      sprintf(buf,"ERROR:%d",errno);
...
streng *std_linein( tsd_t *TSD, cparamboxptr parms )
{
   ...
   else
   {
      if (!line)
         flush_input( ptr ) ;
#ifdef REXX_8BIT_SUPPORT
      res = Str_convert_TSD(streng_clone_value(streng_alloc_base_value(nullstringptr())));
#else
      res = nullstringptr() ;
#endif
   }

   return Str_convert_TSD(res) ;
}
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(file);
   streng_clone_value(file);
   Str_restore_TSD(file);
#endif
...
static streng *reset_file( tsd_t *TSD, fileboxptr fileptr )
{
   ...
   if (fileptr->fileptr)
      return Str_convert_TSD(Str_creTSD( "READY" )) ;   /* Per definition */
   else
      return Str_convert_TSD(Str_creTSD( "UNKNOWN" )) ;
}
...
streng *dbg_dumpfiles( tsd_t *TSD, cparamboxptr parms )
{
   ...
   if ( opt == 'N' )
   {
      ...
      {
         fno = fileno( ptr->fileptr ) ;
         fprintf( TSD->stddump,"%4d %-30s", fno, Str_val(ptr->filename0)/*ptr->filename0->value*/);
         i = 0 ;
         ...
         {
            if (ptr->errmsg)
               fprintf(TSD->stddump, "     ==> %s\n", Str_val(ptr->errmsg)/*ptr->errmsg->value*/ ) ;
            else if (ptr->error)
               fprintf(TSD->stddump, "     ==> %s\n", strerror( ptr->error )) ;
         }
      }
      ...
      for ( i = start; i < end; i++ )
      {
         ...
         if ( ptr )
         {
            ...
            fprintf( TSD->stddump,"Slot: %3d: %4d %8lx prev: %8lx next: %8lx %-30s\n",
               i, fno, (long)ptr, (long)ptr->prev, (long)ptr->next, Str_val(ptr->filename0)/*ptr->filename0->value*/);
            for ( ptr = ptr->next; ptr; ptr = ptr->next )
            {
               ...
               fprintf( TSD->stddump,"           %4d %8lx prev: %8lx next: %8lx %-30s\n",
                  fno, (long)ptr, (long)ptr->prev, (long)ptr->next, Str_val(ptr->filename0)/*ptr->filename0->value*/ );
            }
         }
         else
         {
 ...
 */
{
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(source);
   streng_clone_value(source);
   Str_restore_TSD(source);
#endif
   ...
   my_fullpathstreng( TSD, Str_val(result)/*result->value*/, source );
   result->len = strlen( Str_val(result)/*result->value*/ );
                                          ...
                                          const char *inname, FILE **fp )
{
   ...
   *fp = fopen( inname, "rb" );
   ...
   return Str_convert_TSD(Str_crestrTSD( buf ));
}
...
void CloseOpenFiles( const tsd_t *TSD, FilePtrDisposition fpd )
{
   ...
   FFLUSH(yyout_io);
   if ( !TSD->interactive && fpd != fpdRETAIN ) {
      if ( yyout_io ) {
         FCLOSE(yyout_io);
      }
      yyout_io = 0;
...
streng *ConfigStreamQualified( tsd_t *TSD, const streng *name )
{
   ...
   if ( my_fullpath( TSD, Str_val(result)/*result->value*/, fn ) == -1 )
   {
      /*
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(src);
   streng_clone_value(src);
#endif
