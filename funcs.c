   ...
   if (!func)
   {
      ...
      else
      {
         ...
         while ((end)&&(up>=low))
         {
            mid = (up+low)/2 ;
            for (i=0; i<topp; i++ )
               if (Str_val(thisptr->name)[i] /*thisptr->name->value[i]*/ != functions[mid].funcname[i])
                  break ;

            if (i==topp)
               end = (functions[mid].funcname[i]!=0x00) ;
            else
               end = ( functions[mid].funcname[i] - Str_val(thisptr->name)[i] /*thisptr->name->value[i]*/ ) ;
...
char getoptionchar( tsd_t *TSD, const streng *text, const char* bif, int argnum, const char *ansi_choices, const char *regina_choices )
{
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(text);
   streng_clone_value(text);
   Str_restore_TSD(text);
#endif
   ...
   ch = (char) rx_toupper( Str_val(text)[0]/*text->value[0]*/ ) ;
   /*
...
char getonechar( tsd_t *TSD, const streng *text, const char *bif, int argnum )
{
   ...
   return Str_val(text)[0]/*text->value[0]*/ ;
}

char getoneorigchar( tsd_t *TSD, const streng *text, const char *bif, int argnum )
{
   if ( !text ) {
      exiterror( ERR_INCORRECT_CALL, 23, bif, argnum, "" )  ;
      if ( TSD->interactive ) {
   ...
   return Str_origval(text)[0]/*text->value[0]*/ ;
}
...
streng *int_to_streng( const tsd_t *TSD, int input )
{
   ...
   streng_clone_value(output);
#endif

   start = Str_val(output) /*output->value*/ ;
   ...
   if (input)
   {
      ...
      output->len = top-cptr + start-Str_val(output) /*output->value*/ ;
   }
   else
   {
      *start = '0' ;
...
#ifdef REXX_8BIT_SUPPORT
   streng_clone_base_value(output);
#endif

   return Str_convert_TSD(output) ;
}
...
int convert_date(tsd_t *TSD, const streng *suppdate, char suppformat, struct tm *indate, char isep)
{
   ...
   char buf[20];
   char *ptr=(char*)Str_val(suppdate)/*suppdate->value*/;
   struct tm *tmpTime;
   time_t num64;
...
int convert_time( const tsd_t *TSD, const streng *supptime, char suppformat, struct tm *intime, time_t *unow)
{
   ...
   char buf[20];
   char *ptr=(char*)Str_val(supptime)/*supptime->value*/;
   struct tm *tmpTime;
   time_t num64;
