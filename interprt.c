               ...
               switch( thisptr->p[0]->p[i]->type )
               {
                  case X_DO_TO:
                  {
                     ...
                     if ( !myiswnumber( TSD, chptr, &s.stopval, 1 ) ) {
                        exiterror( ERR_INVALID_INTEGER, (thisptr->p[0]->p[i]->type==X_DO_EXPR) ? 2 : 3, Str_val(chptr)/*chptr->value*/ );
                        if ( TSD->interactive ) {
                            return 0;
                        }
                     }
 ...
 */
      ...
      case X_SAY:
      {
         ...
         if (ok==HOOK_GO_ON)
         {
            if (stringen)
            {
                ...
                * Bug: 1455211
                */
               char *buf = Str_val(stringen)/*stringen->value*/;
               long done,chunk;
...
#else
               /*
                * Here we write 'value' to output descriptor
                */
               FWRITE( Str_origval(stringen)/*stringen->value*/, Str_len(stringen), 1, yyout_io);
               FFLUSH( yyout_io);
                  ...
                  if (nptr->type == X_IND_SYMBOL)
                  {
                     ...
                     for (end = 0;;)
                     {
                        ...
                        while ((begin < Str_len(value)) &&
                               rx_isspace(Str_val(value)[begin]/*value->value[begin]*/))
                           begin++;
                        if (begin == Str_len(value))
                           break;
                        end = begin + 1; /* find next separator */
                        while ((end < Str_len(value)) &&
                               !rx_isspace(Str_val(value)[end]/*value->value[end]*/))
                           end++;
                        /* end now on space after word or past end of string */
                        name = Str_makeTSD(end - begin);
                        name->len = end - begin;
                        memcpy(Str_val(name)/*name->value*/, Str_val(value)/*value->value*/ + begin, Str_len(name));
                        Str_upper(name);
                        drop_var( TSD, name ) ;
                        Free_stringTSD( name ) ;
                     }
      ...
      case X_PARSE:
      {
         ...
         else
         {
            ...
            switch ( thisptr->p[0]->type )
            {
               ...
               case X_PARSE_PULL:
                  //printf("X_PARSE_PULL %p %p\n", user_handlers.TsoPullHandler, user_handlers.TsoPullArg);
                  if ( user_handlers.TsoPullHandler ) {
                     char out_buf[1024];

                     user_handlers.TsoPullHandler(user_handlers.TsoPullArg, "PARSE_PULL", out_buf, sizeof(out_buf));
                     source = Str_creTSD(out_buf);
                  } else {
               ...
               case X_PARSE_VER:
                  //printf("X_PARSE_VER %p %p\n", user_handlers.ParseVersionHandler, user_handlers.ParseVersionArg);
                  if ( user_handlers.ParseVersionHandler ) {
                      char out_buf[1024];
                      user_handlers.ParseVersionHandler(user_handlers.ParseVersionArg, "PARSE VERSION", out_buf, sizeof(out_buf));
                      source = Str_creTSD(out_buf);
                  } else {
               ...
               case X_PARSE_SRC:
               {
                  //printf("X_PARSE_SRC %p %p\n", user_handlers.ParseSourceHandler, user_handlers.ParseSourceArg);
                  if ( user_handlers.ParseSourceHandler ) {
                      char out_buf[1024];
                      user_handlers.ParseSourceHandler(user_handlers.ParseSourceArg, "PARSE SOURCE", out_buf, sizeof(out_buf));
                      source = Str_creTSD(out_buf);
                  } else {
                      const char *stype ;
                      streng *inpfile;
               ...
               case X_PARSE_NUM:
                  //printf("X_PARSE_NUM %p %p\n", user_handlers.ParseNumericHandler, user_handlers.ParseNumericArg);
                  if ( user_handlers.ParseNumericHandler ) {
                      char out_buf[1024];
                      user_handlers.ParseNumericHandler(user_handlers.ParseNumericArg, "PARSE NUMERIC", out_buf, sizeof(out_buf));
                      source = Str_creTSD(out_buf);
                  } else {
                      source = Str_creTSD("NOT SUPPORTED");
                  }
                  break ;
            }
      ...
      case X_PULL:
      {
          ...
          if ( user_handlers.TsoPullHandler ) {
              ...
              user_handlers.TsoPullHandler(user_handlers.TsoPullArg, "PULL", out_buf, sizeof(out_buf));

              doparse(TSD, stmp=Str_upper(Str_creTSD(out_buf)), thisptr->p[0], 0 ) ;
              Free_stringTSD( stmp ) ;
          } else {
              streng *stmp ;
      ...
      case X_PUSH:
          //printf("X_PUSH %p %p\n", user_handlers.TsoPushHandler, user_handlers.TsoPushArg);
          if ( user_handlers.TsoPushHandler ) {
              streng *line = (thisptr->p[0]) ? evaluate(TSD,thisptr->p[0],NULL) : nullstringptr();
              user_handlers.TsoPushHandler(user_handlers.TsoPushArg, "PUSH", Str_origval(line), Str_len(line));
          } else {
      ...
      case X_QUEUE:
          //printf("X_QUEUE %p %p\n", user_handlers.TsoQueueHandler, user_handlers.TsoQueueArg);
          if ( user_handlers.TsoQueueHandler ) {
              streng *line = (thisptr->p[0]) ? evaluate(TSD,thisptr->p[0],NULL) : nullstringptr();
              user_handlers.TsoQueueHandler(user_handlers.TsoQueueArg, "QUEUE", Str_origval(line), Str_len(line));
          } else {
...
nodeptr getlabel( const tsd_t *TSD, const streng *name )
{
   ...
   if (ipt->sort_labels == NULL)
   {
      ...
      for (i = 0, lptr = ipt->first_label;i < ipt->numlabels;i++)
         {
            lptr->hash = hashvalue_ic(Str_val(lptr->entry->name)/*lptr->entry->name->value*/, lptr->entry->name->len);
   ...
   hash = hashvalue_ic(Str_val(name) /*name->value*/, name->len);
...
static void expose_indir( tsd_t *TSD, const streng *list )
{
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(list);
   streng_clone_value(list);
   Str_restore_TSD(list);
#endif

   cptr = Str_val(list) /*list->value*/ ;
   eptr = cptr + list->len ;
   tmp = Str_makeTSD( 64 ) ;

#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(tmp);
   streng_clone_value(tmp);
   Str_convert_TSD(tmp);
#endif
   ...
   for (;cptr<eptr;)
   {
      ...
      memcpy( Str_val(tmp) /*tmp->value*/, sptr, cptr-sptr ) ;
      tmp->len = cptr-sptr ;
