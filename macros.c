                    ...
                    int ctype )
{
   ...
   if ( setjmp( *jbuf ) )
   {
      ...
      if ( !TSD->instore_is_errorfree )
      {
          ...
          */
         if ( RetCode )
            *RetCode = atoi( Str_val(ptr)/*ptr->value*/ );
                          ...
                          int * volatile RetCode, int hooks, int ctype )
{
   ...
   else
   {
      ...
      if ( ( command->len == 7 )
      && ( memcmp( "<stdin>", Str_val(command)/*command->value*/, command->len ) == 0 ) )
      {
         fptr = stdin;
      ...
      else
      {
         ...
         if (! io_handlers.programFetchHandler)
             name = get_external_routine( TSD, path, &fptr );
         else  {
             char* retname = io_handlers.programFetchHandler(io_handlers.programFetchHandlerArg, path, &fptr);
             if (retname)
            	 name = Str_convert_TSD(Str_crestrTSD(retname));
             else  {
                 name = NULL;
                 set_options_flag( TSD->currlevel, EXT_EXT_COMMANDS_AS_FUNCS, FALSE);
             }
         }
      ...
      if ( fptr && fptr != stdin )                  // RRT 15Apr2016
         fclose( fptr );
      if ( parsing.result == 0 )
      {
