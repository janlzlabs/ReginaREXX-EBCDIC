...
typedef struct _tsd_t {
   ...
   /* The following flags are for interactive mode */
   int                    interactive;           /* when REXX runs in interactive mode,
                                                  * i.e. a syntax error will cause an abort
                                                  */
   int                    wait_input;            /* means what it says, so we don't show the prompt */
   int                    do_exit;               /* to mark exit action */
   jmp_buf                interactive_return;    /* jump buffer to return to next command processing */
   streng *               one_arg;               /* when we are called with "arg arg arg" */
   streng *               tokenized_args[64];    /* tokenized one_arg */
   int                    tokenized_count;
} tsd_t;
