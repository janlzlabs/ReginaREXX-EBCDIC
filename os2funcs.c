...
streng *os2_directory( tsd_t *TSD, cparamboxptr parms )
{
   ...
   if (ok==HOOK_GO_ON)
   {
      ...
      my_fullpath( TSD, Str_val(result)/*->value*/, "." );
...
#if defined(DJGPP)
   fnsplit( Str_val(inpath)/*->value*/, fdrive, fdir, fname, fext );
#elif defined(__EMX__)
   _splitpath( Str_val(inpath)/*->value*/, fdrive, fdir, fname, fext );
#elif defined(HAVE__SPLITPATH2)
   _splitpath2( Str_val(inpath)/*->value*/, fpath, &fdrive, &fdir, &fname, &fext );
#elif defined(HAVE__SPLITPATH)
   _splitpath( Str_val(inpath)/*->value*/, fdrive, fdir, fname, fext );
#else
   my_splitpath2( Str_val(inpath)/*->value*/, fpath, &fdrive, &fdir, &fname, &fext );
#endif
   switch( format )
   {
      case 'D':
