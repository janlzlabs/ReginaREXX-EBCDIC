 ...
 *  Copyright (C) 2016 Jan Lisowiec jan.lisowiec@lzlabs.com
 *
...
static int Callback_setenv( const char *name, const char *value )
{
   char *cmd;
...
static int Callback_fork_exec(tsd_t *TSD, environment *env, const char *cmdline, int *rcode)
{
   int rc = -1;
   streng *ret;
   char ret_buf[8];
   ...
   switch (env->subtype)
   {
      case SUBENVIR_SYSTEM:
         if ( io_handlers.subcomSystemHandler ) {
             rc = io_handlers.subcomSystemHandler(io_handlers.subcomSystemHandlerArg, cmdline, strlen(cmdline));
             *rcode = rc;
         }

         {
            //char rc_buf[128];
            //sprintf(rc_buf, "Command %s ret: %d\n", cmdline, rc);

            /*
             * Convert error message, if a converison routine is provided
             */
            //if ( regina_output_conv_handler_get() ) {
                //regina_output_conv_handler_get()(rc_buf, strlen(rc_buf));
            //}

            //FWRITE(rc_buf, strlen(rc_buf), 1, yyout_io);
   ...
   if ( env->output.tmp_queue ) {
       sprintf(ret_buf, "%d", rc);
       ret = Str_creTSD( ret_buf );
 ...
 */
static int Callback_wait(int process)
{
 ...
 */
static int Callback_open_subprocess_connection(const tsd_t *TSD, environpart *ep)
{
   char *name;
   int eno;
 ...
 * handles.
 */
static void Callback_unblock_handle( int *handle, void *async_info )
{
   (handle = handle);
 ...
 */
static void Callback_restart_file(int hdl)
{
 ...
 * watched handles.
 */
static int Callback_close(int handle, void *async_info)
{
   (async_info = async_info);
   return(close(handle));
}
 ...
 */
static void Callback_close_special( int handle )
{
   assert( handle == -1 );
}
 ...
 */
static int Callback_read(int hdl, void *buf, unsigned size, void *async_info)
{
   int done;
 ...
 */
static int Callback_write(int hdl, const void *buf, unsigned size, void *async_info)
{
   int done;
 ...
 */
static void *Callback_create_async_info(const tsd_t *TSD)
{
   (TSD = TSD);
   return(NULL);
}
 ...
 */
static void Callback_delete_async_info(void *async_info)
{
   (async_info = async_info);
}
 ...
 */
static void Callback_reset_async_info(void *async_info)
{
   (async_info = async_info);
}
 ...
 */
static void Callback_add_async_waiter(void *async_info, int handle, int add_as_read_handle)
{
   (async_info = async_info);
   (handle = handle);
 ...
 */
static void Callback_wait_async_info(void *async_info)
{
   (async_info = async_info);
}

int Callback_uname(struct regina_utsname *name)
{
   /*
...
OS_Dep_funcs __regina_OS_Callbacks =
{
   Callback_init,                           /* init                       */
   Callback_setenv,                         /* setenv                     */
   Callback_fork_exec,                      /* fork_exec                  */
   Callback_wait,                           /* wait                       */
   Callback_open_subprocess_connection,     /* open_subprocess_connection */
   Callback_unblock_handle,                 /* unblock_handle             */
   Callback_restart_file,                   /* restart_file               */
   Callback_close,                          /* close                      */
   Callback_close_special,                  /* close_special              */
   Callback_read,                           /* read                       */
   Callback_write,                          /* write                      */
   Callback_create_async_info,              /* create_async_info          */
   Callback_delete_async_info,              /* delete_async_info          */
   Callback_reset_async_info,               /* reset_async_info           */
   Callback_add_async_waiter,               /* add_async_waiter           */
   Callback_wait_async_info,                /* wait_async_info            */
   Callback_uname                           /* uname                      */
};
