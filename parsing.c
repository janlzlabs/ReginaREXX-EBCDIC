...
static const streng nullstring={ 0, 0, 0, "", "" } ;
#else
              ...
              int caseless )
{
   ...
   NeedPtr = (const unsigned char *) Str_val(needle)/*needle->value*/;
   NeedLen = needle->len;

   HeyBase = (const unsigned char *) Str_val(heystack)/*heystack->value*/;
...
#ifdef REXX_8BIT_SUPPORT
       streng_clone_base_value(needle);
       Str_convert_TSD(needle);

       streng_clone_base_value(heystack);
       Str_convert_TSD(heystack);
#endif
...
/*       nextstart = end + Str_len(pattern) ; */
   ...
   if (thisptr->p[0])
   {
      doparse3( TSD, thisptr->p[0], Str_val(source)/*source->value*/+start, end-start);
      --end;
   }
