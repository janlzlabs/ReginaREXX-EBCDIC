...
*/
      ...
      if ( TSD->interactive && TSD->do_exit ) {
          return 0;
      }
   }
   return rexx_optind;
...
static void assign_all_args(tsd_t *TSD, const char* s)
{
    char int_s[1024];
    char* token = 0;
    //int ii;

    TSD->one_arg = Str_makeTSD(strlen(s));
#ifdef REXX_8BIT_SUPPORT
    streng_alloc_base_value(TSD->one_arg);
#endif
    memcpy(Str_val(TSD->one_arg), s, strlen(s));
    memcpy(Str_origval(TSD->one_arg), s, strlen(s));
    TSD->one_arg->len = strlen(s);
    CLEAR_STRENG_CONVERTED(TSD->one_arg->converted);
    Str_convert_TSD(TSD->one_arg);

    strcpy(int_s, Str_val(TSD->one_arg));
    //printf("%s one_arg(%s) tokenized_count(%d)\n", __FUNCTION__, int_s, TSD->tokenized_count);

    for ( token = strtok(int_s, " ") ; token ; token = strtok(0, " ") ) {
        //printf("%s token(%s)\n", __FUNCTION__, token);
        TSD->tokenized_args[TSD->tokenized_count] = Str_makeTSD(strlen(token));
#ifdef REXX_8BIT_SUPPORT
        streng_alloc_base_value(TSD->tokenized_args[TSD->tokenized_count]);
#endif
        memcpy(Str_val(TSD->tokenized_args[TSD->tokenized_count]), token, strlen(token));
        memcpy(Str_origval(TSD->tokenized_args[TSD->tokenized_count]), token, strlen(token));
        TSD->tokenized_args[TSD->tokenized_count]->len = strlen(token);
        CLEAR_STRENG_CONVERTED(TSD->tokenized_args[TSD->tokenized_count]->converted);
        Str_convert_TSD(TSD->tokenized_args[TSD->tokenized_count]);
        TSD->tokenized_count++;
    }

    //for ( ii = 0 ; ii < TSD->tokenized_count ; ii++ ) {
        //printf("%s TSD->tokenized_arg[%d](%s,%s)\n", __FUNCTION__, ii, Str_val(TSD->tokenized_args[ii]), Str_origval(TSD->tokenized_args[ii]));
    //}
}

static void unassign_all_args(tsd_t *TSD)
{
    int ii;
    for ( ii = 0 ; ii < TSD->tokenized_count ; ii++ ) {
        //printf("%s TSD->tokenized_arg[%d](%s,%s)\n", __FUNCTION__, ii, Str_val(TSD->tokenized_args[ii]), Str_origval(TSD->tokenized_args[ii]));
        TSD->tokenized_args[ii] = 0;
    }
    TSD->tokenized_count = 0;
    TSD->one_arg = 0;
}

/*
 * assign_arg sets the program ARG to the string provided
 */

static void assign_arg(tsd_t *TSD, const char* s)
{
	int         len = strlen(s);
	paramboxptr args;
	streng*     str;

	args = TSD->currlevel->args = (paramboxptr) MallocTSD(sizeof(parambox));
	memset(args, 0, sizeof(parambox));

	args->value = str = Str_makeTSD(len);
	str = Str_catstrTSD(str, s);

	assign_all_args(TSD, s);
}
...
#endif
{
   ...
   if ( setjmp( jbuf ) )
   {
      ...
      {
         /* Check if the open files have been closed and close */
         FFLUSH(yyout_io);
   ...
   else
   {
      ...
      char* input_file_specific = getenv("REGINA_INPUT_FILE");
      char* input_file_default  = "<stdin>";
      char* input_file;
...
// RRT 15Apr2016
         if (input_file_specific)
             input_file = input_file_specific;
         else
             input_file = input_file_default;

         TSD->systeminfo->input_file = Str_crestrTSD( input_file );
// RRT 15Apr2016
      ...
      char* program_arg = getenv("REGINA_PROGRAM_ARG");

      if (program_arg)
    	  assign_arg(TSD, program_arg);
      else
