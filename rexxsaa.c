 ...
 *
 * Description:
 *   This function accepts a string identified by *org and length and loads
 *   it into a RXSTRING.  If the RXSTRING has an associated buffer, that
 *   buffer is loaded and truncation (RXDSTK_RET_TRUNC) may occur. If the
 *   RXSTRING strptr value is NULL, a suitable buffer is allocated exposing
 *   the caller on a bad day to RXDSTK_RET_MEMFL.  
 *
 *   The maxlength argument contains the length of the input RXSTRING:strptr
 *   if not null. On exit RXSTRING:strlength is set appropriately.
 *
 *   The caller is responsible for freeing RXSTRING memory allocated by this
 *   function. IfcAllocateMemory is a wrapped malloc(), free() is fine.
 *
 ****************************************************************************/
static int StringReturn(RXSTRING* rxstr, ULONG maxlength, const char* org, int length)
{
    char*   alloc;
    ...
    if (! rxstr->strptr)  {
        alloc = (char*) IfcAllocateMemory(length);  /* really just a malloc */
        if (! alloc)
            return RXDSTK_RET_MEMFL;

        rxstr->strptr = alloc;
        rxstr->strlength = length;
    }    
   
    rxstr->strlength = length > maxlength ? maxlength : length;
    memcpy(rxstr->strptr, org, rxstr->strlength);

    if (rxstr->strlength < length)
        return RXDSTK_RET_TRUNC;   
        
    return 0;
}

/**************************************************************************** 
 * Function: RexxDataStack 
 * 
 * Description: 
 *   This function implements services for accessing the data stack from an
 *   application program in the spirit of SAA access functions like 
 *   RexxVariablePool below. All requests, inputs and outputs are contained
 *   in a DSTKPARMS block mapped in rexxsaa.h. It follows the form of the
 *   SHVBLOCK used to provide access to variables, but only supports operation
 *   on a single stack line per call. 
 *   
 *   RXDSTK_PUSH    - Rexx PUSH semantics for a given string
 *   RXDSTK_QUEUE   - Rexx QUEUE semantics for a given string
 *   RXDSTK_PULL    - Rexx PULL semantics, destructive read
 *   RXDSTK_QUEUE   - Rexx QUEUED() semantics, return # stacked entries
 *
 * Inputs:
 *   pDatastackParms is a pointer to a DSTKPARMS block containing the following
 *
 *   dstkline       - For PUSH and QUEUE requests, strptr contains the address 
 *                    and strlength the length of entry to place on the stack. 
 *                    dstkmaxline is not referenced.
 *
 *                    For PULL requests, optionally contains the address of a 
 *                    target buffer. Refer to dstkmaxline below. If the strptr 
 *                    component is null a suitable buffer is allocated and must 
 *                    be freed by the caller. This is the recommended approach 
 *                    as there is no redo - PULL requests are destructive.  
 *                   
 *   dstkmaxline    - For PULL requests, contains the size of the buffer 
 *                    addressed by dstkline.strptr if that value is not null.
 *                    If a buffer is provided truncation may occur.
 *                   
 *   dstkcode       - Supply a RXDSTK_ request code as described above.
 *
 *   dstkret        - Somewhat in keeping with RexxVariablePool, this return  
 *                    value conveys status associated with the specific 
 *                    operation attempted. It is valid only when the function
 *                    return code is zero (RXDSTK_RET_OK).
 *
 *                    RXDSTK_RET_TRUNC can be returned on a PULL request when
 *                    the caller explicitly provides a return buffer that is
 *                    not large enough to contain the result. See recomendations
 *                    above. 
 *                    
 *                    RXDSTK_RET_MEMFL indicates a memory allocation/management
 *                    error. Look for a loop!
 *
 * Outputs:
 *   The status code returned by this routine is zero indicating success or
 *   non-zero indicating big trouble, including programming errors.  Note
 *   that both RXFUNC_ and RXDSTK_RET_ are returned ambigously in this manner.
 *   RexxVariablePool suffers the same ambiguities.
 *
 ****************************************************************************/
EXPORT_C APIRET APIENTRY RexxDataStack(PDSTKPARMS pDatastackParms)
{
   PDSTKPARMS  dstk = pDatastackParms;
   rex_tsd_t   *rt;
   tsd_t       *TSD = getGlobalTSD();
   int         reqcode;
   int         rc = 0;
   int         bWriteStack = 0, bReadStack = 0;
   int         auxstatus;               // out
   char        **pstrptr;               // in for reads, out for writes
   ULONG       *pstrlength;             // in for reads, out for writes
   char        *retstr;
   ULONG       retlength;
...
/* what does this next test do ???? */

   if (!TSD->interactive && ! TSD->systeminfo->tree.root)  /* see RexxVarPool ... */
      return RXDSTK_RET_NOAVL ;

   dstk->dstkret = RXDSTK_RET_OK;

   switch(dstk->dstkcode) 
   {
      case RXDSTK_PUSH:
         reqcode = RX_STACK_PUSH;
         bWriteStack = 1;
         break;

      case RXDSTK_QUEUE:   
         reqcode = RX_STACK_QUEUE;
         bWriteStack = 1;
         break;

      case RXDSTK_PULL:
         reqcode = RX_STACK_PULL;
         bReadStack = 1;
         break;
         
      case RXDSTK_QUEUED:
         reqcode = RX_STACK_QUEUED;
         break;
      
      default:
         dstk->dstkret = RXDSTK_RET_BADF;
         break;
   }
...
/* PUSH and QUEUE place entries on the stack */

   if (bWriteStack)  {
      pstrlength = &dstk->dstkline.strlength;
      pstrptr = &dstk->dstkline.strptr;
//    printf("stack write length: %d value: %.*s \n", (int) *pstrlength, (int) *pstrlength, *pstrptr); /* ?????? */
      rc = IfcDataStack(TSD, reqcode, pstrptr, pstrlength, NULL);
      assert(rc == 0);
   } 
 
/* PULL removes entries from the stack */

   else if (bReadStack)  {
      retlength = 0;
      retstr = NULL;
      rc = IfcDataStack(TSD, reqcode, &retstr, &retlength, NULL);
      assert(rc == 0);

      if (! rc)  {
         rc = StringReturn(&dstk->dstkline, dstk->dstkmaxline, retstr, retlength);
         dstk->dstkret = rc;
//       printf("stack read length: %d value: %.*s \n", (int) retlength, (int) retlength, retstr);  /* ???????? */
      }
...
/* QUEUED tests stack contents */

   else  {  
       rc = IfcDataStack(TSD, reqcode, NULL, NULL, &auxstatus);
       assert(rc == 0);
       dstk->dstkret = auxstatus;
   }     
...
EXPORT_C APIRET APIENTRY RexxVariablePool(PSHVBLOCK RequestBlockList )
{
   ...
   if (!TSD->interactive && TSD->systeminfo->tree.root==NULL) /* Doesn't the interpreter run? */
      return RXSHV_NOAVL ;
