...
/*
 * Definitions and declarations for the Rexx data stack interface (RRT)
 */

#ifdef INCL_RXDSTK

# define RXDSTK_RET_OK      0x00    /* Success                      */
# define RXDSTK_RET_TRUNC   0x04    /* PULLed value trunc           */
# define RXDSTK_RET_MEMFL   0x10    /* Memory problem,out of memory */
# define RXDSTK_RET_BADF    0x80    /* Invalid function code        */

# define RXDSTK_RET_NOAVL   0x90    /* Interface is not available ?????? */

/* function codes map to RX_STACK_ macros in rxiface.h */ 

# define RXDSTK_PUSH        1       /* PUSH  record to top */
# define RXDSTK_QUEUE       2       /* QUEUE record tk end */
# define RXDSTK_PULL        3       /* PULL  top record    */
# define RXDSTK_QUEUED      4       /* ???????????????????????????? */ 

typedef struct dstkparms
{
   RXSTRING        dstkline;        /* inbound/outbound line        */
   ULONG           dstkmaxline;     /* allocated length of RXSTRING */
   UCHAR           dstkcode;        /* inbound request code         */
   int             dstkret;         /* outbound status              */
} DSTKPARMS;

typedef DSTKPARMS *PDSTKPARMS; 

APIRET APIENTRY RexxDataStack(PDSTKPARMS DataStackParms) EXTNAME("RexxDataStack");
#define REXXDATASTACK RexxDataStack
