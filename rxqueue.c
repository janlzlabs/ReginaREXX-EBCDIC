...
int main( int argc, char *argv[])
{
   ...
   else if ( num == 1 )
   {
       ...
       * "rxqueue /switch"
       */
      if ( argv[rexx_optind][0] == '/' )
      {
         /*
          * Only parameter is a switch
          */
         in_queue = get_unspecified_queue();
         action = get_action( argv[rexx_optind] );
      }
      else
      {
         /*
   ...
   else if ( num == 2 )
   {
       ...
       */
      in_queue = argv[rexx_optind];
      if ( argv[rexx_optind+1][0] == '/' )
      {
         /*
          * Parameter is a switch
          */
         action = get_action( argv[rexx_optind+1] );
      }
   }
   else
   {
