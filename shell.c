 ...
 */
{
   ...
   leaflen = sprintf( Str_val(e->currname)/*e->currname->value*/ + e->currnamelen, "%d", pos ) ;
 ...
 */
{
   ...
   memcpy( e->currname->value, Str_val(source)/*source->value*/, e->currnamelen ) ;
   ...
   switch (awt)
   {
      ...
      case awtSTEM:
         ...
         else /* must be REPLACE */
         {
            e->maxnum = 0 ;
            e->currnum = 1 ;
            Str_val(e->base)/*e->base->value*/[0] = '0' ;
            e->base->len = 1 ;
...
/* Adds one line to the stem specified in e. */
{
   ...
   e->maxnum = e->currnum ;
   e->currnum++ ;
   e->base->len = sprintf( Str_val(e->base)/*e->base->value*/, "%d", e->maxnum ) ;
   stem_access( TSD, e, 0, Str_dupTSD( e->base ) ) ;
 ...
 */
{
   ...
   done = write_buffered(TSD, hdl, Str_val(*string)/*(*string)->value*/, total, async_info);
   if (done <= 0)
   {
   ...
   {
      (*string)->len -= done ;
      memmove(Str_val(*string)/*(*string)->value*/, Str_val(*string)/*(*string)->value*/ + done, (*string)->len);
 ...
 */
{
...
#ifdef REXX_8BIT_SUPPORT
      streng_alloc_base_value(s);
      streng_clone_value(s);
#endif
   ...
   else
   {
      ...
      if (len + done > total)
      {
         ...
         s->len = len ;
         memcpy( Str_val(s)/*s->value*/, Str_val(*string)/*(*string)->value*/, len ) ;
         Free_stringTSD( *string ) ;
      }
   }
   memcpy( Str_val(s)/*s->value*/ + len, buf, done ) ;
   s->len += done ;
   *string = s ;
   Str_convert_TSD(*string);
   return( 0 ) ;
 ...
 */
{
   ...
   /* Append a newline to the end of the line before returning */
      ...
      memcpy(Str_val(retval)/*retval->value*/, Str_val(c)/*c->value*/, c->len);
      Str_val(retval)/*retval->value*/[c->len] = REGINA_CR;
      Str_val(retvall)/*retval->value*/[c->len + 1] = REGINA_EOL;
      retval->len = c->len + 2;
#else
      ...
      memcpy(Str_val(retval)/*retval->value*/, Str_val(c)/*c->value*/, c->len);
      Str_val(retval)/*retval->value*/[c->len] = REGINA_EOL;
      retval->len = c->len + 1;
#endif
      if (delflag)
         Free_stringTSD( (streng *) c ) ;
 ...
 */
{
   ...
   memcpy( Str_val(string)/*string->value*/, data, length ) ;
   string->len = length ;
   Str_val(string)/*string->value*/[length] = '\0' ;
   ...
   else
   {
      ...
      if (max > 0)
      {
         if (ptr == Str_val(s)/*s->value*/)
         {
            ...
            memmove( Str_val(s)/*s->value*/, ptr, max ) ;
      }
      s->len = max ;
   }
   *string = s ;
...
int posix_do_command( tsd_t *TSD, const streng *command, int io_flags, environment *env, Queue *redir )
{
   ...
   if ( ( child == -1 ) || ( child == 0 ) )
   {
      ...
      return ( rc > 0 ) ? -rc : ( rc == 0 ) ? ( !env->arg ? -1000 : rc) : rc;
   }
