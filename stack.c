...
streng *stack_to_line( const tsd_t *TSD, Queue *q )
{
   ...
   dest = Str_val(retval)/*retval->value*/ ;

   for ( ptr = q->u.t.top; ptr != NULL; )
   {
      memcpy( dest, Str_val(ptr->contents)/*->value*/, Str_len( ptr->contents ) ) ;
   ...
   while ((dest != Str_val(retval)/*retval->value*/) && (dest[-1] == ' '))
      dest--;

   /* finally, terminate the string */
   *dest = '\0';                          /* DON'T increment dest */
   Str_len( retval ) = (int)( dest - Str_val(retval)/*retval->value*/ ) ;
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(stemname);
   streng_clone_value(stemname);
   Str_restore_TSD(stemname);
#endif

   stem = Str_makeTSD(stemlen + 1 + 3*sizeof(int));

#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(stem);
   streng_clone_value(stem);
#endif

   memcpy(Str_val(stem)/*->value*/, Str_val(stemname)/*->value*/, stemlen);

   for (i = 1; i <= stem0; i++)
   {                                    /* Fetch the value:                  */
      stem->len = stemlen + sprintf(Str_val(stem)/*stem->value*/ + stemlen, "%d", i);
...
static int use_external( const tsd_t *TSD, const streng *queue_name )
{
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(queue_name);
   streng_clone_value(queue_name);
   Str_restore_TSD(queue_name);
#endif
   ...
   if ( get_options_flag( TSD->currlevel, EXT_QUEUES_301 ) == 0 )
   {
       ...
       */
      if  ( memchr( Str_val(queue_name)/*->value*/, '@', Str_len( queue_name ) ) == NULL )
         return 0;
   }

#ifdef REXX_8BIT_SUPPORT
   Str_convert_TSD(queue_name);
#endif
...
int stack_lifo( tsd_t *TSD, streng *line, const streng *queue_name )
{
   ...
   streng_alloc_base_value(line);
   streng_clone_value(line);
   Str_restore_TSD(line);
#endif
...
streng *popline( tsd_t *TSD, const streng *queue_name, int *result, unsigned long waitflag )
{
   ...
   if ( !use_external( TSD, queue_name ) )
   {
      if ( queue_name )
      {
         if ( ( q = find_queue( TSD, st, queue_name ) ) == NULL )
         {
            ...
            if ( !TSD->called_from_saa ) {
#ifdef REXX_8BIT_SUPPORT
               contents = Str_convert_TSD(streng_clone_value(streng_alloc_base_value(nullstringptr())));
#else
      ...
      if ( line != NULL )
      {
         ...
         FreeTSD( line ) ;
          ...
          if ( io_handlers.stackReadHandler )  {
              void* stkrdarg = io_handlers.stackReadHandlerArg;
              int   stkrdrc  = io_handlers.stackReadHandler(stkrdarg, &io_retptr, &io_retlen);
              if (stkrdrc == 0) {
                  contents = Str_makeTSD( io_retlen ) ;

#ifdef REXX_8BIT_SUPPORT
                  streng_alloc_base_value(contents);
                  streng_clone_value(contents);
#endif

                  // need to populate streng->value, then base_value will be converted to base encoding, so we set converted bit
                  memcpy( Str_origval(contents)/*contents->value*/, io_retptr, io_retlen ) ;
                  contents->len = io_retlen;
                  SET_STRENG_CONVERTED(contents->converted);

#ifdef REXX_8BIT_SUPPORT
                  streng_clone_value(contents);
                  Str_restore_TSD(contents);
#endif
              }
          }
      }
   ...
   return Str_convert_TSD(contents);
}
...
void type_buffer( tsd_t *TSD )
{
   ...
   q = st->current_queue ;
   name = get_queue( TSD ) ;
   fprintf(TSD->stddump,"==> Name: %.*s\n", Str_len(name), Str_val(name)/*->value*/ ) ;
   ...
   for ( counter = q->u.i.buffers, b = q->u.i.top; b != NULL; b = b->lower )
   {
      ...
      for ( ptr = b->top; ptr != NULL; ptr = ptr->lower )
      {
         ...
         for ( cptr = Str_val(ptr->contents)/*->value*/ ; cptr < stop; cptr++ )
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(new_name);
   streng_clone_value(new_name);
   Str_restore_TSD(new_name);

   streng_alloc_base_value(retval);
   streng_clone_value(retval);
   Str_restore_TSD(retval);

   Str_convert_TSD(new_name);
   Str_convert_TSD(retval);
#endif
...
int timeout_queue( tsd_t *TSD, const streng *timeout, const streng *queue_name )
{
   ...
   streng_alloc_base_value(timeout);
   streng_clone_value(timeout);
   Str_restore_TSD(timeout);
#endif
...
streng *set_queue( tsd_t *TSD, const streng *queue_name )
{
   ...
   if ( !use_external( TSD, queue_name ) )
   {
      ...
      assert( ( q->type == QisSESSION ) || ( q->type == QisInternal ) ) ;
      return Str_convert_TSD(SetCurrentQueue( TSD, st, q, Str_dupTSD( q->u.i.name ) ));
   }
#if !defined(NO_EXTERNAL_QUEUES)
   else
   {
      ...
      if ( ( rc = set_queue_in_rxstack( TSD, work->u.e.socket, base ) ) == 0 )
      {
         ...
         return Str_convert_TSD(SetCurrentQueue( TSD, st, work, result ));
      }
      Free_stringTSD( base );
      disconnect_from_rxstack( TSD, &q ) ;
      assert( !TSD->called_from_saa );
