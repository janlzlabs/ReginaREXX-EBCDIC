...
streng *Str_ndup_TSD( const tsd_t *TSD, const streng *name, int length )
{
   ...
   memcpy( ptr->base_value, name->base_value, length );
   ptr->converted = name->converted;
#endif

#ifdef REXXDEBUG
   fprintf(stderr, "func=%s\n", __FUNCTION__);
#endif
   return ptr; //Str_convert_TSD(ptr);
}
...
#ifdef REXX_8BIT_SUPPORT
 streng_alloc_base_value(ptr);
 memcpy( ptr->base_value, name->base_value + offset, length );
 ptr->converted = name->converted;
#endif

#ifdef REXXDEBUG
#ifdef REXX_8BIT_SUPPORT
   fprintf(stderr, "func=%s %d %d\n", __FUNCTION__, ptr->converted, name->converted);
#else
   fprintf(stderr, "func=%s\n", __FUNCTION__);
#endif
#endif
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(output);
   memcpy( output->base_value, input->base_value, input->len );
   output->converted = input->converted;
#endif

#ifdef REXXDEBUG
   fprintf(stderr, "func=%s\n", __FUNCTION__);
#endif
   return output; //Str_convert_TSD(output);
}
...
#ifdef REXX_8BIT_SUPPORT
   streng_realloc_base_value(newstr);
   memcpy( newstr->base_value, from->base_value, length );
   newstr->converted = from->converted;
#endif

#ifdef REXXDEBUG
   fprintf(stderr, "func=%s\n", __FUNCTION__);
#endif
   return Str_convert_TSD(newstr);
}
...
streng *Str_ncre_TSD( const tsd_t *TSD, const char *from, int length )
{
   ...
   print_stre(stderr, "ASCII", result);
   fprintf(stderr, "\n");
#endif
...
#ifdef REXX_8BIT_SUPPORT
   result->converted = 0;
   CLEAR_STRENG_CONVERTED(result->converted);
   CLEAR_STRENG_RESTORED(result->converted);
#endif
#ifdef REXXDEBUG_STRING
   fprintf(stderr, "func=%s ", __FUNCTION__);
   print_stre(stderr, "ASCII",  result);
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(ptr);
   streng_clone_from_streng_base_value(ptr, str);
   ptr->converted = str->converted;
#endif
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(base);
   streng_clone_value(base);
   Str_restore_TSD(base);
#endif
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(ptr);
   memcpy( ptr->base_value + tmp_b, append, tmp );
...
streng *Str_nocat_TSD( const tsd_t *TSD, streng *first, const streng *second, int length, int offset )
{
   ...
   streng_alloc_base_value(second);
   streng_clone_value(second);
   Str_restore_TSD(second);
#endif
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(ptr);
   memcpy( &ptr->base_value[first->len - tmp], &second->base_value[offset], tmp );
   streng_clone_base_value(ptr);
#endif
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(ptr);
   memcpy( ptr->base_value + tmp_f, second->base_value, Str_len( second ) );
   streng_clone_base_value(ptr);
#endif

#ifdef REXXDEBUG
#ifdef REXX_8BIT_SUPPORT
   fprintf(stderr, "func=%s %d %d\n", __FUNCTION__, first->converted, second->converted);
#else
   fprintf(stderr, "func=%s\n", __FUNCTION__);
#endif
#endif
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(ptr);
   memcpy( ptr->base_value + tmp_f, second->base_value, tmp );
   streng_clone_base_value(ptr);
#endif
...
#ifdef REXX_8BIT_SUPPORT
   fprintf(stderr, "func=%s\n", __FUNCTION__);
   debug_print_hex_buf(stderr, Str_val(first), Str_len(first));
   debug_print_hex_buf(stderr, Str_val(second), Str_len(second));
#else
   fprintf(stderr, "func=%s\n", __FUNCTION__);
   debug_print_hex_buf(stderr, Str_val(first), Str_len(first));
   debug_print_hex_buf(stderr, Str_val(second), Str_len(second));
#endif
#endif
   return memcmp( Str_val(first)/*first->value*/, Str_val(second)/*second->value*/, lim );
}
...
int Str_ccmp( const streng *first, const streng *second )
{
...
#ifdef REXXDEBUG
#ifdef REXX_8BIT_SUPPORT
   fprintf(stderr, "func=%s %d %d  %d %d\n", __FUNCTION__,
       first->converted, second->converted, Str_len(first), Str_len(second));
   debug_print_hex_buf(stderr, Str_val(first), Str_len(first));
   debug_print_hex_buf(stderr, Str_val(second), Str_len(second));
#else
   fprintf(stderr, "func=%s %d %d\n", __FUNCTION__, Str_len(first), Str_len(second));
   debug_print_hex_buf(stderr, Str_val(first), Str_len(first));
   debug_print_hex_buf(stderr, Str_val(second), Str_len(second));
#endif
#endif
   ...
   return ( mem_cmpic( Str_val(first) /* first->value */, Str_val(second) /* second->value */, lim ) ) ? 1 : 0;
}
...
int Str_cncmp( const streng *first, const streng *second, int length )
{
   ...
   return ( mem_cmpic( Str_val(first) /* first->value */, Str_val(second) /* second->value */, top ) ) ? 1 : 0;
}
...
int Str_cnocmp( const streng *first, const streng *second, int length, int offset )
{
   ...
   top = MIN( shorter, length );
   return ( mem_cmpic( Str_val(first) /* first->value */, Str_val(second) /* second->value */ + offset, top ) ) ? 1 : 0;
}
...
char *str_of( const tsd_t *TSD, const streng *input )
{
   ...
   Str_convert_TSD(input);
#endif

   char *retval = (char *)MallocTSD( Str_len( input ) + 1 );

   memcpy( retval, Str_val(input) /*input->value*/, Str_len( input ) );
...
#ifdef REXX_8BIT_SUPPORT
   streng_realloc_base_value(ptr);
   memcpy( ptr->base_value + Str_len( ptr ) - top, input, top );
   CLEAR_STRENG_CONVERTED(ptr->converted);
#endif

#ifdef REXXDEBUG
#ifdef REXX_8BIT_SUPPORT
   fprintf(stderr, "func=%s %d %d\n", __FUNCTION__, ptr->converted, base->converted);
#else
   fprintf(stderr, "func=%s\n", __FUNCTION__);
#endif
#endif
...
streng *Str_upper( streng *input )
{
     ...
     * TODO
     */
   assert ( input->base_value );
   mem_upper( input->base_value, Str_len( input ) );
   //streng_alloc_base_value(input);
   streng_clone_base_value(input);
#else
...
streng *Str_lower( streng *input )
{
     ...
     */
   assert ( input->base_value );
   mem_lower( input->base_value, Str_len( input ) );
   //streng_alloc_base_value(input);
   streng_clone_base_value(input);
#else
...
streng *Str_strp( streng *input , char chr, char opt )
{
   ...
   if ( opt & STRIP_TRAILING )
   {
      ...
      {
         if ( Str_val(input)[i]/*input->value[i]*/ != chr )
            break;
      }
      input->len = i + 1;
   }
   if ( opt & STRIP_LEADING )
   {
      for ( j = 0; j < input->len; j++ )
         if ( Str_val(input)[j]/*input->value[j]*/ != chr )
            break;
      for ( i = 0; j < input->len; i++, j++ )
         Str_val(input)[i]/*input->value[i]*/ = Str_val(input)[j]/*input->value[j]*/;
      input->len = i;
   }
