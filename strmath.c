   ...
   if (node)
   {
      switch ( node->type )
      {
         ...
         case X_HEAD_SYMBOL:
            ...
            streng_realloc_base_value(retval);
...
#ifdef REXXDEBUG
   fprintf(stderr, "func=%s (num=%p) ", __FUNCTION__, descr);
   print_stre(stderr, "ASCII", num);
   fprintf(stderr, "\n");
#endif
   /*
...
streng *str_norm( const tsd_t *TSD, num_descr *in, streng *trystr )
{
   ...
   if ( in->size == 0 )
   {
      ...
      if ( trystr )
      {
         if ( trystr->max )
         {
            trystr->value[0] = '0';
#ifdef REXX_8BIT_SUPPORT
            streng_alloc_base_value(trystr);
            trystr->base_value[0] = '0';
#endif
            trystr->len = 1;
         }
         else
...
#ifdef REXX_8BIT_SUPPORT
      streng_clone_base_value(trystr);
#endif

      return Str_convert_TSD(trystr);
   }
   ...
   memcpy( Str_val(result) /*result->value*/, mt->norm_out, size );
...
int str_true( const tsd_t *TSD, const streng *input )
{
   ...
   switch (Str_val(input)[0] /*->value[0]*/)
   {
      case '1':
         return 1 ;
      case '0':
...
streng *str_abs( tsd_t *TSD, const streng *input )
{
   ...
   if ( get_options_flag( TSD->currlevel, EXT_STRICT_ANSI ) )
   {
      ...
      return Str_convert_TSD(str_norm( TSD, &mt->fdescr, NULL ));
   }
...
streng *str_sign( tsd_t *TSD, const streng *input )
{
   ...
   for ( i = 0; i < mt->fdescr.size; i++ )
   {
      if ( mant[i] != '0' )
      {
         ...
         {
            return Str_convert_TSD(Str_creTSD( "-1" ));
         }
         else
         {
            return Str_convert_TSD(Str_creTSD( "1" ));
   ...
   return Str_convert_TSD(Str_creTSD( "0" ));
}
...
streng *str_trunc( tsd_t *TSD, const streng *number, int deci )
{
   ...
   for (i=0; i<top; i++)
      ...
      Str_val(result)[j++] = mt->fdescr.num[i] ;
...
#ifdef REXX_8BIT_SUPPORT
   //streng_realloc_base_value(result);
   streng_clone_base_value(result);
                      ...
                      const char *bif, int removeStringOnError )
{
   ...
   for (cur_byte=Str_len(input)-1; cur_byte>=start; cur_byte--)
   {
      for (cur_bit=0; cur_bit<8; cur_bit++)
      {
         ...
         if ((sign) ^ ((Str_val(input)[cur_byte] >> cur_bit) & 1))
         {
            if (too_large)
                exiterror( ERR_INVALID_INTEGER, 0 )  ;
   ...
   return Str_convert_TSD(str_norm( TSD, &mt->edescr, NULL ));
}
...
streng *str_binerize( tsd_t *TSD, num_descr *num, int length, int errnum, int suberrnum, const char *bif, int argno )
{
   ...
   else if ( length == -1 )
   {
      ...
      for ( i = length; ; i-- )
      {
          ...
          */
         /* result->value[i] */ Str_val(result)[i] = (char) descr_to_int( TSD, &mt->fdescr, errnum, suberrnum, bif, argno );
      ...
      memmove( Str_val(result), &Str_val(result)[i], length + 1 - i );
