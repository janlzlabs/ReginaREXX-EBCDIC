...
static void printout( tsd_t *TSD, const streng *message )
{
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(message);
   streng_clone_value(message);
   Str_restore_TSD(message);
#endif
   ...
   if ( rc == HOOK_GO_ON )
   {
      ...
      if ( yyout_io ) {
          //fwrite( Str_val(message)/*message->value*/, message->len, 1, fp ) ;
          FWRITE( Str_origval(message)/*message->value*/, message->len, 1, yyout_io ) ;
      }
      if ( get_options_flag( TSD->currlevel, EXT_TRACE_HTML ) )
         //fwrite( "</PRE></FONT>", 13, 1, fp );
         if ( yyout_io ) {
             FWRITE( "</PRE></FONT>", 13, 1, yyout_io );
         }
...
void traceerror( tsd_t *TSD, const treenode *thisptr, int RC )
{
   ...
   if ( ( TSD->trace_stat == 'E' ) || ( RC < 0 && ( TSD->trace_stat == 'F' || TSD->trace_stat == 'N' ) ) )
   {
      ...
      message->len = sprintf( Str_val(message)/*message->value*/, "       +++ RC=%d +++", RC );

#ifdef REXX_8BIT_SUPPORT
      streng_alloc_base_value(message);
#endif

      printout( TSD, Str_convert_TSD(message) );
                    ...
                    const streng *index, char trch )
{
   ...
   Str_restore_TSD(stem);

   streng_alloc_base_value(index);
   streng_clone_value(index);
   Str_restore_TSD(index);
#endif
   ...
   message->len = sprintf( Str_val(message)/*message->value*/, tt->tracestr,
                                           "", Str_val(stem)/*stem->value*/, Str_val(index)/*index->value*/ );
...
void jump_interactive_return( tsd_t *TSD )
{
#ifdef SUPPORT_INTERACTIVE_JUMP
   longjmp( TSD->protect_return, 2 );
#endif
}
...
int intertrace( tsd_t *TSD )
{
   ...
   for ( ; retvalue1 < 0; )
   {
      ...
      if ( ( Str_len( str ) == 1 ) && (Str_val(str)/*str->value*/[0] == '=' ) )
      {
         tt->traceflag = 0;
         retvalue1 = 1;
      }
...
void tracenumber( tsd_t *TSD, const num_descr *num, char type )
{
   ...
   sprintf( tt->tracestr, "       >%%c> %%%ds  \"%%.%ds\"",
                          indent, tmpstr->len );
   message->len = sprintf( Str_val(message)/*message->value*/, tt->tracestr,
                                           type, "", Str_val(tmpstr)/*tmpstr->value*/ );

#ifdef REXX_8BIT_SUPPORT
   streng_clone_base_value(message);
#endif
...
void tracevalue( tsd_t *TSD, const streng *str, char type )
{
   ...
   for (i=0;i<str->len;i++)
   {
      if ( Str_val(str)/*str->value*/[i] == '\0' )
         Str_val(message)/*message->value*/[message->len++] = ' ';
      else
         Str_val(message)/*message->value*/[message->len++] = Str_val(str)/*str->value*/[i];
   }
   Str_val(message)/*message->value*/[message->len++] = '"';
...
void traceline( tsd_t *TSD, const treenode *thisptr, char tch, int offset )
{
   ...
   else
   {
      sprintf( tt->tracestr, "%%6d *-* %%%ds%%.%ds",
                             indent + offset, srcstr->len );
      message->len = sprintf( Str_val(message)/*message->value*/, tt->tracestr,
                                              thisptr->lineno, "", Str_val(srcstr)/*srcstr->value*/ );
   }
...
void traceback( tsd_t *TSD )
{
   ...
   if ( TSD->currentnode )
   {
      ...
      message->len = sprintf( Str_val(message)/*message->value*/, tt->tracefmt,
                                              TSD->currentnode->lineno, "",
                                                             Str_val(srcline)/*srcline->value*/ );
...
void getcallstack( tsd_t *TSD, streng *stem )
{
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(varname);
   streng_clone_value(varname);
#endif

   memcpy( Str_val(varname)/*varname->value*/, Str_val(stem)/*stem->value*/, stemlen ) ;
   mem_upper( Str_val(varname)/*varname->value*/, stemlen );
   eptr = Str_val(varname)/*varname->value*/ + stemlen ;
   ...
   for ( ss = TSD->systeminfo; ss; ss = ss->previous )
   {
      for ( i = ss->cstackcnt - 1; i >= 0; i-- )
      {
         ...
         memcpy( Str_val(value)/*value->value*/, Str_val(lineno)/*lineno->value*/, lineno->len ) ;
         ...
         /* set the tail value */
         sprintf(eptr, "%d", ++j ) ;
         varname->len = strlen( Str_val(varname)/*varname->value*/ ) ;
         ...
         setvalue( TSD, Str_convert_TSD(varname), Str_convert_TSD(value), -1 ) ;
      }
   }
   *eptr = '0' ;
   ...
   setvalue( TSD, Str_convert_TSD(varname), Str_convert_TSD(tmpptr), -1 ) ;
...
void set_trace( tsd_t *TSD, const streng *setting )
{
   ...
   else
   {
      for ( cptr = 0; cptr < Str_len( setting ); cptr++ )
      {
          ...
          */
         set_trace_char( TSD, Str_val(setting)[cptr]); /* settings->value[cptr] ); */
         if ( rx_isalpha( Str_val(setting)[cptr]) ) /* settings->value[cptr] ) ) */
            break;
      }
   }
