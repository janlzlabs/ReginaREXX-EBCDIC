                           ...
                           streng *value, variableptr *oldptr, unsigned hash)
{
   ...
   if ( value && !GET_STRENG_RESTORED(value->converted) ) {
...
int valid_var_symbol( const streng *name )
{
   ...
   cptr = (const unsigned char *) Str_val(name)/*name->value*/;
   eptr = cptr + name->len;
...
#ifdef REXX_8BIT_SUPPORT
      streng_alloc_base_value(vt->tmpindex);
      streng_clone_value(vt->tmpindex);
#endif
...
static streng *fix_index( tsd_t *TSD, nodeptr thisptr )
{
   ...
   freespc = vt->tmpindex->max ;
      ...
      streng_alloc_base_value(vt->tmpindex);
      streng_clone_value(vt->tmpindex);
      Str_restore_TSD(vt->tmpindex);
#endif

   cptr = Str_val(vt->tmpindex)/*vt->tmpindex->value*/ ;
   ...
   for ( ; ; )
   {
...
#ifdef REXX_8BIT_SUPPORT
      streng_alloc_base_value(large);
      streng_clone_value(large);
#endif

         memcpy( Str_val(large)/*large->value*/, Str_val(vt->tmpindex)/*vt->tmpindex->value*/, (cptr-Str_val(vt->tmpindex)/*vt->tmpindex->value*/)) ;
         cptr = Str_val(large)/*large->value*/ + (cptr-Str_val(vt->tmpindex)/*vt->tmpindex->value*/) ;
      ...
      memcpy( cptr, Str_val(value)/*value->value*/, value->len ) ;
      cptr += value->len ;
      thisptr = thisptr->p[0] ;
      if ( thisptr )
         *(cptr++) = '.' ;
   ...
   vt->tmpindex->len = cptr - Str_val(vt->tmpindex)/*vt->tmpindex->value*/ ;
...
#ifdef REXX_8BIT_SUPPORT
   streng_clone_base_value(vt->tmpindex);
#endif

   return Str_convert_TSD(vt->tmpindex) ;
}
                            ...
                            var_hashtable *vars, int *expand )
{
   ...
   for ( ; ; )
   {
      ...
      {
         cptr = Str_val(vt->tmpindex) /*vt->tmpindex->value*/ + vt->tmpindex->len;
         for ( i = start; i < stop; i++ )
            *cptr++ = (char) rx_toupper( Str_val(name)[i] /*name->value[i]*/ );
         vt->tmpindex->len = cptr - Str_val(vt->tmpindex) /*vt->tmpindex->value*/;
      }
      ...
      start = stop + 1;
      Str_val(vt->tmpindex)[vt->tmpindex->len++] /*vt->tmpindex->value[vt->tmpindex->len++]*/ = '.';
   }
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(indexstr);
   streng_clone_value(indexstr);
   Str_restore_TSD(indexstr);
#endif
...
#ifdef REXX_8BIT_SUPPORT
      streng_alloc_base_value(vt->ovalue);
      streng_clone_value(vt->ovalue);
#endif
                               ...
                               int *isRes )
{
   ...
   if ( KNOWN_RESERVED( Str_val(name) /*name->value*/, Str_len( name ) ) )
   {
      *isRes = 1;
      if ( ( pool == 0 ) || ( pool == -1 ) )
         return vt->pool0;
...
void setvalue( tsd_t *TSD, const streng *name, streng *value, int pool )
{
   ...
   if ( value ) {
       if ( !GET_STRENG_RESTORED(value->converted) ) {
           streng_alloc_base_value(value);
           streng_clone_value(value);
           Str_restore_TSD(value);
       }
       //if ( GET_STRENG_RESTORED(value->converted) && GET_STRENG_CONVERTED(value->converted) ) {
           //SET_STRENG_VARIABLE(value->converted);
       //}
   }
#endif
   ...
   {
      for ( i = 0; ( i < len ) && ( Str_val(name)[i] /*name->value[i]*/ != '.' ); i++ )
         ;
...
void setdirvalue( tsd_t *TSD, const streng *name, streng *value )
{
   ...
   if ( KNOWN_RESERVED( Str_val(name)/*name->value*/, len ) )
   {
      vt = (var_tsd_t *)TSD->var_tsd;
...
const streng *getdirvalue( tsd_t *TSD, const streng *name )
{
   ...
   if ( ( i = KNOWN_RESERVED( Str_val(name)/*name->value*/, len ) ) != 0 )
   {
      vt = (var_tsd_t *)TSD->var_tsd;
...
#ifdef REXX_8BIT_SUPPORT
   if ( indexstr && !GET_STRENG_RESTORED(indexstr->converted) ) {
       streng_alloc_base_value(indexstr);
       streng_clone_value(indexstr);
       Str_restore_TSD(indexstr);
   }
...
#ifdef REXX_8BIT_SUPPORT
         if ( result && !GET_STRENG_RESTORED(result->converted) ) {
...
#ifdef REXX_8BIT_SUPPORT
   if ( resstr && !GET_STRENG_RESTORED(resstr->converted) ) {
       streng_alloc_base_value(resstr);
       streng_clone_value(resstr);
       Str_restore_TSD(resstr);
   }
#endif
...
#ifdef REXX_8BIT_SUPPORT
   if ( string_val && !GET_STRENG_RESTORED(string_val->converted) ) {
       streng_alloc_base_value(string_val);
       streng_clone_value(string_val);
       Str_restore_TSD(string_val);
   }
#endif
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(thisptr->name);
   streng_clone_value(thisptr->name);
   Str_restore_TSD(thisptr->name);
#endif
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(indeks);
   streng_clone_value(indeks);
   Str_restore_TSD(indeks);
#endif
...
#ifdef REXX_8BIT_SUPPORT
      streng_alloc_base_value(vt->xvalue);
      streng_clone_value(vt->xvalue);
#endif
...
#ifdef REXX_8BIT_SUPPORT
      streng_alloc_base_value(newname);
      streng_clone_value(newname);
#endif
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(val_str);
   streng_clone_value(val_str);
   Str_restore_TSD(val_str);
#endif
