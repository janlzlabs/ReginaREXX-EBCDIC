...
static void write_out_ast()
{
   ...
   if (! stack_empty( TSD )) {
      ...
      rc = sys$qio(0, vt->ichan, IO$_WRITEVBLK, &vt->istat,
                   write_out_ast, 0, Str_val(vt->kill)/*vt->kill->value*/, Str_len(vt->kill), 0, 0,
                   0, 0 ) ;
#     ifdef VMS_DEBUG
         printf( "O" ) ;
         fflush( stdout ) ;
...
int vms_do_command( tsd_t *TSD, const streng *cmd, int io_flags, environment *env, Queue *redir )
{
   ...
   name.dsc$a_pointer = (char *)Str_val(cmd)/*cmd->value*/ ;
...
streng *vms_resolv_symbol( tsd_t *TSD, streng *name, streng *new, streng *pool )
{
   ...
   if (strncmp( Str_val(pool)/*pool->value*/, "SYMBOL", MAX(6,Str_len(pool))) ||
       strncmp( Str_val(pool)/*pool->value*/, "SYSTEM", MAX(6,Str_len(pool))))
   {
      rc = lib$get_symbol( &sym_name, &sym_val, &length ) ;
      if (new)
         lib$set_symbol( &sym_name, &new_val ) ;
   }
   else if (strncmp( Str_val(pool)/*pool->value*/, "LOGICAL", MAX(7, Str_len(pool))))
   {
/*    rc = lib$get_logical( ... ) */
      if (new)
         lib$set_symbol( &sym_name, &new_val ) ;
