...
#define ADD_CHAR(a,b) Str_val((a))/*(a)->value*/[(a)->len++] = (b)
...
static streng *internal_id( const tsd_t *TSD, const short *id )
{
   ...
   sprintf( Str_val(result)/*result->value*/, "(%d,%d,%d)", id[0], id[1], id[2] ) ;
...
static int name_to_num( const tsd_t *TSD, const streng *name )
{
   ...
   descr.dsc$a_pointer = (char *)Str_val(name)/*name->value*/ ;
...
static streng *num_to_name( const tsd_t *TSD, const int num )
{
   ...
   if (glength > -1)
   {
      ...
      Str_val(result)/*result->value*/[result->len++] = ',' ;
   }
   else {
...
static streng *get_uic( const tsd_t *TSD, const union uicdef *uic )
{
   ...
   {
      sprintf(Str_val(result)/*result->value*/,"[%o,%o]", uic->uic$v_group, uic->uic$v_member) ;
      result->len = strlen( Str_val(result)/*result->value*/ ) ;
   }
   return Str_convert_TSD(result) ;
}
...
/*       sprintf( Str_val(result)/*result->value*/, ((type==TYP_INT) ? "%d" : "%u"), *iptr ) ; */
         sprintf( Str_val(result)/*result->value*/, "%d", *iptr ) ;  /* DCL-bug */
         result->len = strlen( Str_val(result)/*result->value*/ ) ;
      ...
      case TYP_LHEX:
      {
         ...
         for (i=0; i<length; i++)
        {
            Str_val(result)/*result->value*/[i*2] = HEX_DIGIT((buffer[length-i-1] >> 4) & 0x0f);
            Str_val(result)/*result->value*/[i*2+1] = HEX_DIGIT(buffer[length-i-1] & 0x0f) ;
         }
         result->len = length * 2 ;
         break ;
      }
      ...
      case TYP_DTIM:
      {
         ...
         result->len = 16 ;
         sprintf( Str_val(result)/*result->value*/, "%4d %02d:%02d:%02d.%02d",
      ...
      case TYP_TIME:
      {
         ...
         desc.dsc$a_pointer = Str_val(result)/*result->value*/ ;
      ...
      case TYP_HEX:
         ...
         streng_clone_value(result);
            ...
            sprintf( Str_val(result)/*result->value*/, "%08X", *iptr ) ;
            result->len = strlen( Str_val(result)/*result->value*/ ) ;
         }
         else
...
static streng *strip_nulls( streng *input )
{
   ...
   for (i=input->len-1; (i>=0) && isspace(Str_val(input)/*input->value*/[i]); i--) ;
   input->len = i+1 ;
   return input ;
}
...
static unsigned int read_pid( const streng *hexpid )
{
   ...
   for (i=0; i<hexpid->len; i++)
      if (isxdigit(Str_val(hexpid)/*hexpid->value*/[i]))
         sum = sum*16 + HEXDIG( Str_val(hexpid)/*hexpid->value*/[i] ) ;
...
streng *vms_f_getdvi( tsd_t *TSD, cparamboxptr parms )
{
   ...
   /*
    * Note that format_results already converts streng
    */
 ...
 */
   ...
   if (objid = (tmp && tmp->next) ? ((tmp=tmp->next)->value) : NULL )
   {
      ...
      else
      {
         ...
         items[cnt++] = (int)&(Str_val(objid)/*objid->value*/[0]) ;
...
streng *vms_f_getsyi( tsd_t *TSD, cparamboxptr parms )
{
   ...
   if (parms->next && parms->next->value)
   {
      ...
      name.dsc$b_dtype = DSC$K_DTYPE_T ;
      name.dsc$b_class = DSC$K_CLASS_S ;
      name.dsc$a_pointer = Str_val(parms->value)/*parms->value->value*/ ;
   }
   ...
   /*
    * Note that format_result already converts steng
    */
   return format_result( TSD, ptr->type, buffer, length ) ;
}
...
streng *vms_f_identifier( tsd_t *TSD, cparamboxptr parms )
{
   ...
   if (!strncmp(Str_val(type)/*type->value*/, "NAME_TO_NUMBER", 14))
   ...
   /*
    * Note that int_to_streng already converts streng
    */
   return result ;
}
...
#ifdef REXX_8BIT_SUPPORT
   val=Str_makeTSD(10));
   streng_alloc_base_value(val);
   streng_clone_value(val);
   sprintf(Str_val(val), "%08x", pid ) ;
#else
...
#ifdef REXX_8BIT_SUPPORT
   return Str_convert_TSD(Str_ncatstrTSD( streng_clone_value(streng_alloc_base_value(Str_makeTSD(ptr-buffer))), buffer, (ptr-buffer))) ;
#else
...
static int extract_privs( int *vector, const streng *privs )
{
   ...
   eptr = Str_end( privs ) ;
   for (ptr=Str_val(privs)/*privs->value*/; ptr<eptr; ptr=(++lptr) )
   {
...
streng *vms_f_privilege( tsd_t *TSD, cparamboxptr parms )
{
   ...
   return Str_convert_TSD(Str_creTSD(
...
streng *vms_f_process( tsd_t *TSD, cparamboxptr parms )
{
   ...
   return Str_convert_TSD(Str_ncatstrTSD( Str_makeTSD(length), buffer, length )) ;
}
...
/*   return str_norm( TSD, parms->value ) ;   / * if it existed */
   return Str_convert_TSD(Str_dupTSD(parms->value)) ;
}
...
streng *vms_f_time( tsd_t *TSD, cparamboxptr parms )
{
   ...
   return Str_convert_TSD(Str_ncatstrTSD( Str_makeTSD(DAT_TIME_LEN+1), buffer, DAT_TIME_LEN)) ;
}
...
streng *vms_f_setprv( tsd_t *TSD, cparamboxptr parms )
{
   ...
   /*
    * Note that map_privs already converts treng
    */
...
streng *vms_f_user( tsd_t *TSD, cparamboxptr parms )
{
   ...
   /*
    * Note that get_uic already converts streng
    */
...
streng *vms_f_trnlnm( tsd_t *TSD, cparamboxptr parms )
{
   ...
   ptr = parms ;
   lognam.dsc$a_pointer = Str_val(ptr->value)/*ptr->value->value*/ ;
   lognam.dsc$w_length = ptr->value->len ;

   if (ptr) ptr=ptr->next ;
   if (ptr && ptr->value)
   {
      tabnam.dsc$a_pointer = Str_val(ptr->value)/*ptr->value->value*/ ;
   ...
   if (item_ptr && item_ptr->type == TYP_TRNM)
   {
      for (cnt=0; cnt<sizeof(trnlnm_modes)/sizeof(struct dvi_items_type);cnt++)
         if (trnlnm_modes[cnt].addr == (*((unsigned char*)buffer)))
            return Str_convert_TSD(Str_creTSD( trnlnm_modes[cnt].name )) ;
      exiterror( ERR_SYSTEM_FAILURE , 0 ) ;
   }

   if (item_ptr && item_ptr->type == TYP_FLAG)
      return Str_convert_TSD(Str_creTSD( (*((int*)buffer) & item_ptr->addr) ? "TRUE" : "FALSE" )) ;
...
streng *vms_f_parse( tsd_t *TSD, cparamboxptr parms )
{
   ...
   fab.fab$l_fna = Str_val(ptr->value)/*ptr->value->value*/ ;
 ...
 */
   ...
   if (ptr && ptr->value)
   {
      fab.fab$l_dna = Str_val(ptr->value)/*ptr->value->value*/ ;
   ...
   if (ptr && ptr->value)
   {
      ...
      relnam.nam$l_rsa = Str_val(ptr->value)/*ptr->value->value*/ ;
   ...
   memcpy( Str_val(result)/*result->value*/, cptr, result->len=clen ) ;
...
streng *vms_f_search( tsd_t *TSD, cparamboxptr parms )
{
   ...
   if (context==0 && ((name->len!=fptr->box->fab$b_fns) ||
                      memcmp(Str_val(name)/*name->value*/, fptr->box->fab$l_fna, name->len )))
      fptr->box->fab$l_nam->nam$b_rsl = 0 ;
...
/*      fptr->box->fab$l_dna = NULL ;
      ...
      fptr->box->fab$l_fna = Str_val(fptr->name)/*fptr->name->value*/ ;
...
/*      fptr->box->fab$l_nam->nam$b_nop = NAM$M_PWD ;
   ...
   memcpy( Str_val(result)/*result->value*/, fptr->box->fab$l_nam->nam$l_rsa, result->len ) ;
...
streng *vms_f_type( tsd_t *TSD, cparamboxptr parms )
{
   ...
   return Str_convert_TSD(Str_creTSD(myisinteger( parms->value ) ? "INTEGER" : "STRING" )) ;
}


static streng *boolean( const tsd_t *TSD, const int param )
{
   return Str_convert_TSD(Str_creTSD( param ? "TRUE" : "FALSE" )) ;
}
...
streng *vms_f_file_attributes( tsd_t *TSD, cparamboxptr parms )
{
   ...
   fab.fab$l_fna = Str_val(parms->value)/*parms->value->value*/ ;
...
#ifdef REXX_8BIT_SUPPORT
        streng_alloc_base_value(res);
        streng_clone_value(res);
#endif

        memcpy( Str_val(res)/*res->value*/, &(nam.nam$t_dvi[1]), res->len=nam.nam$t_dvi[0] ) ;
        break ;
...
streng *vms_f_extract( tsd_t *TSD, cparamboxptr parms )
{
   ...
   memcpy( Str_val(result)/*result->value*/, Str_val(result)/*string->value*/+start, length ) ;
   result->len = length ;
...
streng *vms_f_element( tsd_t *TSD, cparamboxptr parms )
{
   ...
   if (count<number)
   {
      ...
      result->len = 1 ;
      Str_val(result)/*result->value*/[0] = delim ;
   }
   else
   {
      ...
      memcpy( Str_val(result)/*result->value*/, cptr, cmax-cptr ) ;
   }
...
static streng *convert_bin( tsd_t *TSD, cparamboxptr parms, const int issigned, const char *bif )
{
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(temp);
   streng_clone_value(temp);
#endif

   obyte = (start+length)/8 + 1 ;
   temp->len = obyte + 1 ;
   obit = 7 ;
   for (count=0; count<=obyte; Str_val(temp)/*temp->value*/[count++] = 0x00) ;

   for (count=start+length-1; count>=start; count--)
   {
      bit = (Str_val(string)/*string->value*/[count/8] >> (7-(count%8))) & 1 ;
      Str_val(temp)/*temp->value*/[obyte] |= bit << (7-obit--) ;
      if (obit<0)
      {
         obit = 7 ;
         obyte-- ;
   ...
   if (issigned && bit)
      for (;obyte>=0;)
      {
         Str_val(temp)/*temp->value*/[obyte] |= 1 << (7-obit--) ;
         if (obit<0)
         {
            obit = 7 ;
            obyte-- ;
...
streng *vms_f_cvtime( tsd_t *TSD, cparamboxptr parms )
{
   ...
   if (item)
   {
      for (cnt=0; cnt<item->len; cnt++)
         Str_val(item)/*item->value*/[cnt] = toupper(Str_val(item)/*item->value*/[cnt]) ;

      if (item->len==4 && !memcmp(Str_val(item)/*item->value*/, "YEAR", 4))
         func = year ;
      else if (item->len==5 && !memcmp(Str_val(item)/*item->value*/, "MONTH", 5))
         func = month ;
      else if (item->len==8 && !memcmp(Str_val(item)/*item->value*/, "DATETIME", 8))
         func = datetime ;
      else if (item->len==3 && !memcmp(Str_val(item)/*item->value*/, "DAY", 3))
         func = day ;
      else if (item->len==4 && !memcmp(Str_val(item)/*item->value*/, "DATE", 4))
         func = date_part ;
      else if (item->len==4 && !memcmp(Str_val(item)/*item->value*/, "TIME", 4))
         func = time_part ;
      else if (item->len==4 && !memcmp(Str_val(item)/*item->value*/, "HOUR", 4))
         func = hour ;
      else if (item->len==6 && !memcmp(Str_val(item)/*item->value*/, "SECOND", 6))
         func = second ;
      else if (item->len==6 && !memcmp(Str_val(item)/*item->value*/, "MINUTE", 6))
         func = minute ;
      else if (item->len==9 && !memcmp(Str_val(item)/*item->value*/, "HUNDREDTH", 9))
         func = hundredth ;
      else if (item->len==7 && !memcmp(Str_val(item)/*item->value*/, "WEEKDAY", 7))
         func = weekday ;
      else
         exiterror( ERR_INCORRECT_CALL , 0 ) ;
   }
   ...
   if (output)
   {
      for (cnt=0; cnt<output->len; cnt++)
         Str_val(output)/*output->value*/[cnt] = toupper(Str_val(output)/*output->value*/[cnt]) ;

      if (output->len==5 && !memcmp(Str_val(output)/*output->value*/, "DELTA", 5))
         out = delta ;
      else if (output->len==10 && !memcmp(Str_val(output)/*output->value*/, "COMPARISON", 10))
         abs = 0 ;
      else if (output->len==8 && !memcmp(Str_val(output)/*output->value*/, "ABSOLUTE", 8))
         abs = 1 ;
      else
         exiterror( ERR_INCORRECT_CALL , 0 ) ;
   }
   ...
   if (input)
   {
      ...
      cptr = Str_val(input)/*input->value*/ ;
      cend = cptr + input->len ;
   ...
   switch (func)
   {
      case year:
         ...
         sprintf( Str_val(result)/*result->value*/, ((abs) ? "%04d" : "%d"), timearray[func]);
         result->len = strlen( Str_val(result)/*result->value*/ ) ;
         break ;
      ...
      case day:
         ...
         sprintf( Str_val(result)/*result->value*/, ((abs) ? "%d" : "%02d"), timearray[func]);
         result->len = strlen( Str_val(result)/*result->value*/ ) ;
         break ;
      ...
      case month:
         ...
         else
         {
            ...
            sprintf( Str_val(result)/*result->value*/, "%02d", timearray[month]) ;
            result->len = 2 ;
         }
         break ;
      ...
      case date_part:
         ...
         if (out==delta)
            sprintf( Str_val(result)/*result->value*/, "%d", timearray[day] ) ;
         else if (abs)
            sprintf( str_val(result)/*result->value*/, "%d-%s-%d", timearray[day],
                vms_months[timearray[month]], timearray[year] ) ;
         else
            sprintf( Str_val(result)/*result->value*/, "%04d-%02d-%02d", timearray[year],
      ...
      case datetime:
         ...
         if (out==delta)
            sprintf( Str_val(result)/*result->value*/, "%d %02d:%02d:%02d.%02d",
                   timearray[day], timearray[hour], timearray[minute],
                   timearray[second], timearray[hundredth] ) ;
         else if (abs)
            sprintf( Str_val(result)/*result->value*/, "%d-%s-%d %02d:%02d:%02d.%02d",
                   timearray[day], vms_months[timearray[month]],
                   timearray[year], timearray[hour], timearray[minute],
                   timearray[second], timearray[hundredth] ) ;
         else
            sprintf( Str_val(result)/*result->value*/, "%04d-%02d-%02d %02d:%02d:%02d.%02d",
...
streng *vms_f_fao( tsd_t *TSD, cparamboxptr parms )
{
   ...
   ctrl.dsc$a_pointer = Str_val(parms->value)/*parms->value->value*/ ;
   ctrl.dsc$w_length = parms->value->len ;

   cptr = cstart = Str_val(parms->value)/*parms->value->value*/ ;
...
/*      return nullstringptr() ; */
   ...
   result->len = outlen ;
   memcpy( Str_val(result)/*result->value*/, buffer, outlen ) ;
