...
void *wrapper_load( const tsd_t *TSD, const streng *module )
{
   ...
   streng_alloc_base_value(module);
   streng_clone_value(module);
   Str_restore_TSD(module);
#endif
   ...
   memcpy(udpart, Str_val(module)/*module->value*/, Str_len(module) );
...
# endif
   ...
   if ( addon_dir )
   {
      ...
      memcpy(ptmp, Str_val(module)/*module->value*/, Str_len(module) );
...
#ifdef REXX_8BIT_SUPPORT
   streng_alloc_base_value(us_func);
   streng_clone_value(us_func);
#endif

   memcpy( Str_val(us_func)/*us_func->value*/, "_", 1 );
   us_func->len = 1;
