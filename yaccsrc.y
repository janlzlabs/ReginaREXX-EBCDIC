...
%token LABEL DOVARIABLE HEXSTRING STRING VERSION LINEIN WHATEVER NAME GUWNERIC
...
address_stat2: VALUE expr naddr_with   { current->type = X_ADDR_V ;
             ...
             | error                   { EXITERROR( ERR_STRING_EXPECTED, 1, __reginatext ) ;}
               naddr_with
             | nvir nexpr naddr_with   { current->name = STR_CONVERT_TSD((streng *)$1) ;
...
resources    : STREAM nnvir            { /* ANSI extension: nsimsymb is
             ...
             | STEM nsimsymb           {
                                         streng *tmp = STR_CONVERT_TSD($2->name);
                                         char *p;
...
dovar        : DOVARIABLE              { $$ = (nodeptr)STR_CONVERT_TSD(Str_cre_TSD(parser_data.TSD,retvalue)) ; }
             ;
...
labelname    : label                   { $$ = $1 ;
                                         $$->name = STR_CONVERT_TSD(Str_cre_TSD(parser_data.TSD,retvalue)) ; }
             ;
...
parse_param  : ARG                     { $$ = makenode(X_PARSE_ARG,0) ; }
             ...
             | GUWNERIC                { $$ = makenode(X_PARSE_NUM,0) ; }
...
expr        : '('                      { /* We have to accept exprs here even
            ...
            | HEXSTRING                { $$ = makenode( X_STRING, 0 );
                                         ...
                                         streng_clone_value($$->name);
#endif
                                         memcpy(Str_origval($$->name)/*$$->name->value*/,retvalue,
                                                    $$->name->len=retlength);
#ifdef REXX_8BIT_SUPPORT
                                         streng_clone_value($$->name);
                                         SET_STRENG_RESTORED($$->name->converted);
                                         SET_STRENG_CONVERTED($$->name->converted);
#endif
                                         STR_CONVERT_TSD($$->name); }
...
intfunc     : INFUNCNAME               { $$ = (nodeptr)STR_CONVERT_TSD(Str_cre_TSD(parser_data.TSD,retvalue)) ; }
            ;

extfunc     : EXFUNCNAME               { $$ = (nodeptr)STR_CONVERT_TSD(Str_cre_TSD(parser_data.TSD,retvalue)) ; }
            ;
...
offset      : OFFSET                   { $$ = (nodeptr)STR_CONVERT_TSD(Str_cre_TSD(parser_data.TSD,retvalue)) ; }
            | CONSYMBOL                { streng *sptr = STR_CONVERT_TSD(Str_cre_TSD(parser_data.TSD,retvalue)) ;
...
string      : STRING                   { $$ = (nodeptr) STR_CONVERT_TSD(Str_cre_TSD(parser_data.TSD,retvalue)) ; }
...
xsimsymb    : SIMSYMBOL                { $$ = (treenode *) STR_CONVERT_TSD(Str_cre_TSD(parser_data.TSD,retvalue));}
            ;

simsymb     : SIMSYMBOL                { $$ = (treenode *) STR_CONVERT_TSD(Str_cre_TSD(parser_data.TSD,retvalue));}
